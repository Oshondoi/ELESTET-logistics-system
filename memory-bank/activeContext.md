...
## CURRENT FOCUS — 25.09.2026: связно согласуемая редакция 12

- Активное обсуждение `Заявки клиентов, передача партий и фактическая приёмка` находится в редакции 12. Содержательный текст редакции 11 сохранён без изменений: 39 нумерованных пунктов плюс нулевой. Редакция 12 только фиксирует согласование нулевого пункта целиком — самого пункта и всех его 20 строк; всего сохранено 42 состояния.
- Текущая модель: обычная заявка сразу получает `R` и резервирует `P`; передача существующей партии использует тот же `P/S/B` и системные QR без копий. Заявитель управляет только заявкой, получатель работает с клиентской партией как ограниченный исполнитель.
...
- Обычная заявка навсегда занимает `P-ID`; до подтверждения пустой черновик разрешено удалить без возврата номера. Подтверждение активирует партию, скрытый ID исходной заявки и первую стадию исполнителя.
...
- Заявка редактируется без отдельного запроса изменения, но каждое сохранение остаётся в истории и не меняет активную партию автоматически. Аудит охватывает изменения данных, а не пассивные просмотры.
...
- UI discussion production: компактная неподвижная шапка, внутренний scroll контента, цветовая вертикальная навигация с автопрокруткой, локальный черновик галочек и единое явное сохранение опубликованы до текущей редакции 12.
