...
- На этапе fulfillment `Короба` текст кнопки справа от переключателя `FBO / FBS` изменён с `+ Поставка` на `Создать`. Обработчик создания, проверки и выбранный склад не менялись.
...
- На этапе `Короба` между поиском склада назначения и кнопкой `+ Поставка` добавлен переключатель `FBO / FBS`.
