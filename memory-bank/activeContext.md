...
## CURRENT FOCUS — 09.09.2026: быстрый FBS-синк v2 (только локально, не опубликован)
...
- Миграция: `supabase/patch_fbs_fast_sync_v2.sql`. Она ещё не применялась. Edge Function и frontend также не публиковались и не коммитились.
- Локально `npm run build` проходит. Перед публикацией нужны применение миграции, deploy Edge Function, публикация frontend, включение одного тестового магазина и фактическая canary-проверка.
...
## 2026-09-04 — официальная история FBS-поставок (подготовлено, не опубликовано)
...
- В production изменения не применены: текущая авторизация Supabase CLI/Management API возвращает `Unauthorized`. До применения SQL и Edge Function код нельзя отправлять в Vercel, потому что интерфейс уже вызывает новый RPC.

## 2026-09-09 — управляемый каталог FBS-сканеров (локально, не опубликовано)
...
- PostgREST schema cache принудительно обновлён. Production build и `git diff --check` проходят. Фронтенд остаётся только локально, пуш не выполнялся.
