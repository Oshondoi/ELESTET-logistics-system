...
### Реализовано
- Формат паллетоместа: `C{company}_W{warehouse}_R{rack}_S{side}_{pallet}`, коробоместа: тот же код + `_K{slot}`. Пример: `C52_W1_R1_S1_A1_K6`.
- Складам и стеллажам добавлены защищённые порядковые `short_id`; сторона представлена стабильной позицией S1/S2, паллетоместо — адресом A1, номер коробоместа — K1…Kn.
...
### Принятая модель
- В текущем WMS зона является целым стеллажом: `Склад → Стеллаж/зона → Паллетоместо B3 → Сторона S1/S2 → Место K5 → Короб с QR → Товар`.
...
- SQL `supabase/patch_wms_rack_layout.sql` применён к production Supabase; стеллажи «Север» и «Центральный» проверены как `S1/S2`, каждая сторона `2×4 = 8`; обе RPC сохранения планировки и переноса/обмена установлены.
