...
- Актуальный ответ редактируется на месте, а Supabase автоматически сохраняет каждую прежнюю ревизию. Доступны `Снять ответ`, просмотр истории и `Вернуть в актуальные`; модалка истории имеет фиксированные `90vh × 80vw`.
...
- Новый относительно предыдущего содержательно отличающегося ответа текст подсвечивается светло-синим как Excel-действие `Загрузить содержимое`; согласованное остаётся зелёным, метки `Новое/Добавлено` сохраняют происхождение.
- История построена тремя колонками: редакции, узкая вертикальная навигация цифрами, контент. В актуальной карточке есть такая же sticky-навигация. Активная цифра синяя; scroll-spy, плавный переход и автопрокрутка цифрового списка работают в обоих местах.
