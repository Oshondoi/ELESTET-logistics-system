...
- История изменяющих Excel-загрузок и строгая серверная проверка `fulfillment_manage` подготовлены локально в `b86d7c9`, но ещё не production: сначала нужно применить `supabase/patch_fulfillment_excel_action_history.sql`, затем отправить frontend. Скачивания не журналируются, автоматического восстановления прошлой версии нет.
