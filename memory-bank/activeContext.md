...
## CURRENT FOCUS — 26.09.2026: реализован уровень `Реализовано` в структурной модели обсуждений

- Ось согласования расширена до `В обсуждении / Согласовано / Реализовано`. Последний уровень означает фактически выполненное и проверенное требование, выставляется только ИИ после полной сверки актуальной редакции и не имеет ручного чекбокса.
- Блок со строками агрегирует только строки: серый, когда реализованы все; зелёный/синий, когда все минимум согласованы; обсуждаемый, если хотя бы одна строка не согласована. Изменение собственного заголовка или пояснения не сбрасывает агрегат строк.
- Блок без строк согласуется самостоятельно. Его поясняющий текст входит в подпись статуса и при изменении сбрасывает его; `##`-заголовок в подпись согласования не входит, поэтому простое переименование статус сохраняет.
- Чёрная схема остаётся зависимым поясняющим элементом: имеет собственное происхождение, но наследует все уровни согласования смыслового родителя и не влияет на его вычисление. Перед отметкой родителя как реализованного ИИ всё равно обязан проверить схему.
- `DiscussionsTab.tsx` принимает `implemented` и `implemented:<hash>`, сохраняет реализованный статус неизменённых и перемещённых строк, сбрасывает его у изменённых требований и использует серые оттенки во всех представлениях статуса.

## CURRENT FOCUS — 26.09.2026: согласована точная модель происхождения и согласования обсуждений

- Утверждена формула `Статус = Происхождение × Согласование`. Блок, каждая строка и каждая чёрная схема вычисляют происхождение независимо; старый блок может содержать новые, изменённые или перемещённые дочерние элементы.
- Собственный текст блока — заголовок и обычные поясняющие абзацы. Изменение дочерней строки или схемы не меняет происхождение блока; перенумерация после удаления предыдущего блока также ничего не меняет.
- Строка получает `Перемещено` только при смене родительского блока; перестановка внутри одного блока нейтральна. Перемещённая строка сохраняет согласование, а одновременное изменение отмечается как `Перемещено и изменено`.
- Чёрная схема всегда принадлежит блоку либо конкретной строке, имеет собственное происхождение и наследует согласование родителя. Смена родителя означает перенос; схема остаётся чёрной, состояние показывает цветная левая полоса.
- Первая содержательная версия целиком новая не из-за номера, а из-за отсутствия предшественника. История обязательна к загрузке до вычисления цветов; ошибка загрузки не может подменяться отсутствием версии.
- Ручной `item_states` всегда учитывается. Последнее явное решение владельца в чате приоритетнее старого или ручного согласования только при реальном противоречии; все связанные статусы тогда пересматриваются, остальные сохраняются. Явно согласованное новое предложение сразу создаётся с согласованным статусом.
- Модель реализована локально в `DiscussionsTab.tsx`: единый структурный разбор строит независимые блоки, строки и схемы; сравнение различает новое, изменённое, перемещённое и перемещённое с изменением; legacy-ключи согласования нормализуются без потери ручных отметок.
- История всех карточек теперь загружается обязательно до показа контента; ошибка запроса блокирует ложную отрисовку. Первая содержательная версия определяется отсутствием предшественника и целиком получает новое происхождение.
- TypeScript и production build проходят. Frontend опубликован через commit `ba05232`; production `elestet.net` отдаёт проверенный asset `assets/index-CHTFiz6l.js` с новой структурной моделью.
- Активное обсуждение `public-request-invite-auth-20260926` безопасно пересоздано транзакцией после успешного `ROLLBACK`-теста: старый UUID `5f0f2f6f-9f10-40bd-8215-c315a310d312` удалён, новый UUID `871b467c-0680-4a9d-a90d-3ad8172f0ee6`; ключ, заголовок, позиция и 2185 символов текста сохранены. Новая запись имеет `active`, редакцию 1, 0 архивных ревизий и 0 ручных отметок.

## CURRENT FOCUS — 26.09.2026: заявки внедрены в production, обсуждение остаётся активным

- Согласованная предметная редакция 28 реализована в коде и production Supabase миграцией `supabase/migrations/20260926090000_service_requests.sql`: заявки `R`, отдельные партии `P` по магазинным вкладкам, постоянные связи и независимые карточки магазинов, локальные `A-ID`, версии корректировок, приём/отклонение, копирование, приглашения, акты и счета.
- В production подтверждены 8 таблиц и 9 функций контура заявок, необходимые поля магазинов/партий, защита физического удаления и отсутствие действующих магазинов без `A-ID`.
- Полный smoke-test выполнен в транзакции с `ROLLBACK`: черновик → подтверждение/отправка → создание `P` → принятие исполнителем → связь и карточка магазина → акт и счёт. Выявленный конфликт внутренней смены стадии исправлен через локальный `app.pipeline_internal`; повторный тест успешен, тестовых строк не осталось.
- Вкладка `Обсуждение` использует независимые оси происхождения и согласования: оранжевый, фиолетовый, синий и зелёный отражают обсуждение/согласование, серые оттенки — проверенную реализацию. Состояния применяются к пунктам, строкам, навигации, бейджам и полосам чёрных схем.
- Кнопка `Снять ответ` переименована в `Завершить редакцию`. Команда владельца `закрыть редакцию` равнозначна; обновление редакции не завершает её. Текущее обсуждение остаётся актуальным и этой работой не закрывается.
- Функциональные commits `f3d0de2`, `52fb83c` и финализирующий `ab92cab` отправлены в `origin/main`. Vercel production на `elestet.net` после автодеплоя отдаёт asset `assets/index-C0b2uXCj.js`, в котором подтверждена новая кнопка завершения редакции.

## CURRENT FOCUS — 25.09.2026: связно согласуемая редакция 17

- Активное обсуждение `Заявки клиентов, передача партий и фактическая приёмка` находится в редакции 17: полный текст сохранён, 40 нумерованных пунктов плюс нулевой; полученные решения сразу отмечены согласованными. Пункт 8 согласован целиком, ошибочно добавленный сценарий переноса магазина между компаниями удалён.
- Текущая модель: черновик заявки сразу получает постоянный `R`, но `P` появляется только при подтверждении и отправке исполнителю. До принятия исполнителем заявитель может удалить партию и повторно отправить ту же `R` с новой `P`; после принятия операционная партия принадлежит исполнителю, сохраняя `C/P/S/B` и ШК в ID-контексте заявителя.
- Минимальная заявка не требует от заявителя создавать поставки или выбирать магазин: недостающие поставки и распределение по складам выполняет исполнитель от имени заказчика. Незарегистрированным заявителям заранее выдаются постоянные `U/C`, которые активируются подтверждением почты без перенумерации.
- Черновики заявки, приёмки и сканов сохраняются в рабочем слое БД без архивной истории. Первое подтверждение создаёт историю; каждая последующая подтверждённая корректировка обновляет `Заявлено`, сохраняет прежнюю версию и не переписывает `Принято`.
- Одна заявка имеет одного основного исполнителя. Отправка существующей партии заявкой создаёт следующую стадию получателя без копирования `P/S/B/ШК` и без настройки заявителем полного pipeline; каждая компания редактирует только свою стадию, чужую видит read-only.
- `A-N` означает `Applicant`: это короткий ID магазина-заказчика внутри его родительской компании `stores.account_id`. Счётчик отдельный для каждой компании, как у партий; `store_code` остаётся отдельным полем. При первом внедрении нумеруются только 77 действующих магазинов, 33 уже удалённых не затрагиваются.
- Ссылку на заявку создаёт исполнитель; она действует 30 дней. При первом открытии заявитель обязан придумать пароль до перехода к форме. До первого подтверждения заявки существуют только приглашение и гостевая сессия; `U/C/email` создаются и привязываются при подтверждении. До привязки почты забытый пароль не восстанавливается, после привязки сбрасывается по email.
- Полная почта и статус её подтверждения видны в детальной карточке входящей заявки сотруднику исполнителя с доступом к заявке, а владельцу сервиса, superadmin и уполномоченной поддержке — в административной карточке. В общих списках почта не выводится; на сбросе пароля заявителю показывается только маскированный адрес.
- Пользователь с подтверждённой почтой обязан иметь стабильный `device_id` и назвать устройство. Заявитель без подтверждённой почты является единственным исключением: `device_id` ему не присваивается и имя устройства не требуется.
- Согласована целевая возможность скрыть/вернуть партию отдельно для списка каждой компании и переключать скрытые партии кнопкой-глазом; в production эта функция ещё не реализована.
- Постоянное правило вкладки `Обсуждение`: содержательная версия является цельным снимком. Любое решение должно в той же редакции обновить все зависимые пункты; взаимоисключающая старая логика остаётся только в истории, а не в актуальном тексте.
- UI discussion production: компактная неподвижная шапка, внутренний scroll контента, цветовая вертикальная навигация с автопрокруткой, локальный черновик галочек и единое явное сохранение опубликованы до текущей редакции 17.

## CURRENT FOCUS — 24.09.2026: версиями управляемое обсуждение архитектуры

- На `/tz-prompts` первым табом добавлено `Обсуждение`; далее идут `Задачи` и `Промпты ТЗ`. В обсуждении одновременно существует один актуальный ответ и архив завершённых.
- Актуальный ответ редактируется на месте, а Supabase автоматически сохраняет каждую прежнюю ревизию. Доступны `Завершить редакцию`, просмотр истории и `Вернуть в актуальные`; модалка истории имеет фиксированные `90vh × 80vw`.
- Блоки `##` и их маркированные/нумерованные строки имеют уровни `В обсуждении / Согласовано / Реализовано`. Галочки владельца управляют первыми двумя уровнями; `Реализовано` устанавливает только ИИ. Состояния сохраняются в `item_states` текущей записи и ревизии. Чёрные code-блоки наследуют статус родителя.
- Происхождение текста и согласование независимы: новый/изменённый согласованный текст синий, новый/изменённый обсуждаемый — фиолетовый, прежний согласованный — зелёный, прежний обсуждаемый — оранжевый; метки `Новое/Добавлено/Изменено` сохраняют происхождение.
- История построена тремя колонками: редакции, узкая вертикальная навигация цифрами, контент. В актуальной карточке есть такая же sticky-навигация. Активная цифра использует более тёмный оттенок состояния своего пункта; scroll-spy, плавный переход и автопрокрутка цифрового списка работают в обоих местах.
- Галочка на сайте является официальным состоянием согласования, но ИИ не получает её автоматически. Перед следующим ответом по этой предметной теме необходимо перечитать актуальную запись и `item_states` из Supabase.
- Таблицы `tz_discussions`/`tz_discussion_revisions`, триггер истории, RLS и `item_states` применены в production. Frontend опубликован последовательностью commit от `2f2a673` до `d9ebe9b`; последний production asset проверен напрямую.
- Первый активный ответ описывает заявки клиентов, их связь с зарезервированными или существующими партиями, ссылки/доступ, межфирменное исполнение, универсальную фактическую приёмку, готовые короба, документы, подписи, платежи и уведомления. Канонический предметный конспект: `memory-bank/components/intake-requests.md`. С 26.09.2026 это реализованная production-модель.

## CURRENT FOCUS — 24.09.2026: каноническая связь FBO-коробов с WB ШК через Excel

- Базовый бизнес-сценарий начинается с готовых физических коробов и их содержимого в ELESTET; FBO/FBW-поставка на стороне WB в этот момент пустая. Состав коробов из WB не подтягивается и не считается источником истины.
- До официального ответа поддержки WB эталоном соответствия является порядок заполненных пар в однолистном Excel WB: первая непустая пара `ШК короба + ШК короба для печати в стороннем сервисе` относится к коробу №1, вторая — к №2 и далее. Полностью пустые строки пропускаются и номер не занимают. Любые посторонние колонки игнорируются.
- Порядок package codes из WB API признан недостоверным для идентичности физического короба. Автоматическая API-привязка отключена; безопасный sync склада, даты, типа, статуса и количеств поставки сохранён и не меняет WB ШК.
- Импорт требует точного совпадения полного набора номеров с коробами поставки. Лишняя Excel-пара показывается красным, короб без WB-пары — оранжевым; любое расхождение блокирует финальное подтверждение и исключает частичную запись.
- `fulfillment_wb_box_code_registry` хранит обе WB-пары по `supply_id + box_number`. Удаление и повторное создание того же номера возвращает прежние системный и WB ШК. Смена `wb_supply_id` очищает старую WB-идентичность и кэш метаданных, но не системный ШК и не содержимое.
- Зелёная Excel-иконка поставки открывает единое окно с вкладками `Содержимое коробов`, `Загрузить WB ШК`, `Экспорт`. В WB-экспорте доступны только готовые пять колонок шаблона WB и режим `По коробам`; недоступные режимы объясняются подсказкой `Доступно для системных ШК`.
- Production опубликован через commit `551bcc0`; подробная каноническая спецификация находится в `memory-bank/components/wb-excel-export.md`.
- История изменяющих Excel-загрузок и строгая серверная проверка `fulfillment_manage` из `b86d7c9` опубликованы; `supabase/patch_fulfillment_excel_action_history.sql` применён в production до dependent frontend. Скачивания не журналируются, автоматического восстановления прошлой версии нет.
- Общее UI-правило закреплено в `memory-bank/systemPatterns.md`: если модалка имеет подвал, он всегда остаётся у нижнего края; прокручивается только центральная часть. Исключение возможно только по прямому указанию владельца.

## 17.09.2026 — правило исследования трансграна и СПОТ (постоянно)

- Если пользователь просит «изучи детально», «как на самом деле сейчас» или актуальные правила трансграна, не считать внутренний гайд ELESTET источником действующих правил. Сверять его с актуальными первоисточниками: действующей версией руководства и сообщениями Teksher, инструкциями WB для продавцов Кыргызстана и применимыми официальными правилами КР/РФ (включая ФНС/СПОТ). Указывать дату/версию и ссылки; явно перечислять расхождения гайда с действующими правилами и отделять проверенный факт, вывод и неопределённость. Если пользователь прямо просит «по нашему гайду», разбирать гайд как внутренний документ, но отмечать риск устаревания. Автоматическое обновление гайда этим правилом не запрошено.
- Текущая область обсуждения трансграна — только WB, не Ozon и не офлайн. Бизнес-решение пользователя: юридический отправитель — владелец конкретного WB-магазина (ИП/ОсОО и др.); получатель — ООО «РВБ». Юридическую корректность реквизитов и схему FBS подтверждать по первоисточникам перед реализацией.
- Постоянное правило предметной области: **FBO/FBW-поставка и FBS-поставка — разные сущности, их нельзя смешивать**. FBO/FBW — плановая поставка товаров на склад WB с наполнением конкретных коробов; ELESTET хранит её внутреннюю партию/поставку и собственные короба. FBS — поставка собранных заказов покупателей со склада продавца WB; даже одно сборочное задание при стандартной отгрузке входит в FBS-поставку с отдельным ID WB. По инструкции WB при FBS-отгрузке в ПВЗ указывают количество грузомест, но не то, какой заказ в каком коробе; на склад/в СЦ грузоместа могут не требоваться. Грузоместо/QR FBS-поставки не являются FBO-коробом с поштучной раскладкой. При исследовании Текшер, номера документа, упаковки и СПОТ всегда отдельно проверять правила и идентификаторы обеих моделей.
- Для FBO-коробов пользователь утвердил два режима печати — (1) собственный системный ШК или (2) выданный WB ШК. Один числовой WB ID привязывается только на уровне поставки; WB ШК назначаются только проверенным Excel-порядком, а не API. У одиночного короба нет ручной привязки, там только печать уже связанного кода. Выбор источника и отдельное подтверждение печати/скачивания нужны в «Фулфилменте» на уровнях партии, поставки и короба, а также в «Логистике». В БД системный ШК и полная пара WB сохраняются у одного физического короба; на физический короб печатается один выбранный код. Основной процесс, история Excel и усиленная серверная проверка прав опубликованы в production.
- Проверено для FBS-трансграна: WB допускает номер/дату поставки WB или сборочного задания; когда задания собраны в поставку, рабочий основной ключ — ID именно FBS-поставки WB (`WB-GI-…`), не нашего короба/внутренней партии. В коде уже реализованы раздельные источники FBO/FBS, снимок подтверждённых КИЗ, диагностика, создание/синхронизация/отмена операции Teksher и append-only события; СПОТ хранится отдельным процессом. Остаются предметные риски, которые нельзя считать решёнными одной реализацией: внешняя поштучная сверка КИЗ с GTIN/WB/физическим местом и точный фактический заявитель СПОТ по каждой схеме/рейсу при работе через карго. Законодательное правило СПОТ: ДОПП представляет российский получатель/импортёр либо сама площадка, перевозчик получает QR, но не становится заявителем от одной перевозки. Не превращать предположения о конкретном контрагенте в утверждения без подтверждения.
- Проверка первоисточников на 17.09.2026: руководство Teksher v1.7 от 23.06.2026 (`https://main.teksher.kg/files/rukovodstva-polzovatelyz-mzkm-v1.7.pdf`), статья WB для Кыргызстана «Экспорт товаров с КИЗ из Кыргызстана в Россию» от 18.05.2026, раздел ФНС о СПОТ (`https://www.nalog.gov.ru/rn77/related_activities/spot/`). Внутренний `KizGuidePage` всё ещё ссылается на старую версию и не может служить единственным источником. WB в августе 2026 объявил API для данных и QR СПОТ у FBS-поставок продавцов КР: `PUT /api/marketplace/v3/fbs/supplies/{supplyId}/spot`, `POST .../spot/list`, `GET .../stickers/spot`, флаг `spotAvailable` (официальный канал `https://t.me/s/wb_api_notifications?after=452`). Это отдельный процесс от трансграничной операции Teksher и прежними выводами не было учтено.
- По официальной инструкции WB для создания FBW-поставки самостоятельные ШК упаковки допускаются: не `WB_` в начале, без пробелов, уникальные (`https://seller.wildberries.ru/instructions/ru/ru/material/A-716`). Предел 30 символов в найденном правиле WB **не подтверждён**: это внутреннее ограничение системного ШК ELESTET. Текущий WB-экспорт использует только полную пару, загруженную и проверенную из Excel WB; системный экспорт остаётся отдельным режимом. WB рекомендует, чтобы на физическом коробе не было посторонней видимой маркировки.
- Публичное название прямого режима аппаратного сканера во всём UI — только **«Режим ELESTET»**. Термины COM/Web Serial/VCP и подробности способа чтения не показывать пользователю; в Memory Bank технические названия сохранять можно. Это лишь маскировка интерфейса, не полноценная защита интеллектуальной собственности: алгоритм во frontend и выдаваемые клиенту профили всё равно доступны технически подготовленному пользователю.
- Memory Bank нельзя считать закрытым, пока он отслеживается текущим публичным GitHub-репозиторием: удаление файлов из новой версии не удалит их из истории и уже сделанных клонов. Целевое решение — отдельный приватный репозиторий/закрытое хранилище Memory Bank, локально подключённое в рабочую папку по игнорируемому пути; публичный app-репозиторий и деплой Vercel остаются отдельными. Перенос выполнять только после создания приватного назначения и резервной проверки, не удалять историю/не делать force-push без отдельного разрешения пользователя.
- Решение владельца от 21.09.2026: задачу скрытия/переноса Memory Bank отложить бессрочно. Не поднимать её снова и не выполнять никаких изменений по скрытию, пока владелец сам явно не вернётся к вопросу или не возникнет безвыходная техническая необходимость.

## CURRENT FOCUS — 12.09.2026: единый селектор магазина в «Стикеры и КИЗы» и «Товары»

- На табе `Стикеры` в блоке импорта WB и на табе `КИЗы` удалены отдельные кастомные dropdown магазина. Оба места теперь напрямую используют тот же `FbsStoreSelect`, что и страница `FBS Заказы`.
- На странице `Товары`, на активной вкладке `Импорт WB`, прежний кастомный dropdown также заменён на общий `FbsStoreSelect`. Вкладка `Браки` и её отдельный выбор магазина не изменялись.
- Внешний вид и поведение совпадают с FBS: подпись `Магазин`, обычный системный select, placeholder `Выберите магазин`, одинаковые высота, рамка, отступы и violet-focus, единый формат строки `Юридическое название — Название магазина`.
- Источники магазинов не расширялись и не подменялись: импорт стикеров и импорт товаров по-прежнему показывают только магазины с WB API-ключом; КИЗы сохраняют прежний набор и прежнюю сортировку с подключёнными Teksher-магазинами первыми. Удалены только цветные точки и кастомная механика раскрытия.
- На этапе fulfillment `Короба` текст кнопки справа от переключателя `FBO склады / Свой склад` изменён с `+ Поставка` на `Создать`. Обработчик создания, проверки и выбранный склад не менялись.
- Production build проходит (673 модуля); функциональная логика печати, импорта, КИЗов, FBO/FBS и создания поставки не изменялась.

## CURRENT FOCUS — 12.09.2026: раздельный выбор FBO/FBS-склада в fulfillment-поставке

- На этапе `Короба` между поиском склада назначения и кнопкой `Создать` добавлен переключатель с понятными пользователю подписями `FBO склады / Свой склад` (внутренние значения остаются `fbo / fbs`).
- `FBO` показывает только системный справочник складов WB (`warehouses.is_system=true`). Пользовательские строки общего справочника назначения в этот список больше не подмешиваются.
- `FBS` показывает только внутренние склады компании, фактически выполняющей текущую pipeline-стадию, и только при `wms_warehouses.fbs_enabled=true`. Если подходящих складов нет, список прямо сообщает об этом.
- Переключение режима сбрасывает прежний UUID, поиск и открытый dropdown, поэтому склад из другого справочника нельзя случайно сохранить.
- Поставка хранит `destination_type`, FBO-ссылку в `warehouse_id` либо FBS-ссылку в `destination_wms_warehouse_id`; `warehouse_name` остаётся историческим снимком. SQL-проверки и триггер защищают тип, взаимное исключение ссылок, FBS-флаг и принадлежность склада исполнителю стадии.
- Миграция `supabase/patch_fulfillment_supply_destination_types.sql` применена в production. Проверка существующих данных: 143 FBO, 0 FBS до первого нового выбора, 0 некорректных строк. Production build проходит (673 модуля).
- Новое обязательное операционное правило: конкретные данные production-клиента нельзя точечно исправлять вручную без прямой команды владельца. Ошибки должны устраняться общей системной логикой; правило подробно закреплено в `memory-bank/systemPatterns.md`.
...
- `src/types/index.ts`
- `supabase/patch_fulfillment_supply_destination_types.sql`
- `memory-bank/components/supabase-schema.md`
- `memory-bank/systemPatterns.md`

## CURRENT FOCUS — 11.09.2026: быстрый FBS-синк v2 включён глобально

- По прямому решению владельца `fast-v2` включён для всех существующих магазинов (`102/102` production-настроек имеют `true`) и является режимом по умолчанию для будущих магазинов.
- Отсутствие строки в `fbs_sync_settings` теперь означает `fast-v2=true`; явная строка со значением `false` сохранена как аварийный откат отдельного магазина на legacy-контур.
- Быстрое обновление ждёт только новые заказы, активные статусы и одно чтение БД. Поставки обновляются независимым фоновым каналом; проверка КИЗ и повторное чтение не удерживают кнопку.
- Полная сверка не менялась и продолжает использовать полный legacy-контур. Если настройки fast-v2 недоступны из-за серверной ошибки, Edge Function безопасно возвращается на legacy.
- Production изменён патчем `supabase/patch_fbs_fast_sync_v2_global.sql`; Edge Function `wb-fbs` version 62 содержит глобальный default для магазинов без отдельной настройки.

## PREVIOUS FOCUS — 09.09.2026: быстрый FBS-синк v2 (первоначально опубликован выключенным)

- Подготовлен параллельный `fast-v2`, выключенный по умолчанию для каждого магазина. Оплаченные production-клиенты продолжают работать по прежнему пути до явного включения магазина.
- Быстрая кнопка ждёт только новые заказы, статусы активных заказов и одно чтение БД. Поставки обновляются отдельным фоновым каналом; КИЗ, полная история и повторное чтение не задерживают кнопку.
- Убрана повторная проверка через 60 секунд для магазинов на `fast-v2`; открытая вкладка проверяет лёгкий канал примерно раз в 30 секунд. Сервер объединяет параллельные запросы и ограничивает автоматическую частоту.
- После подтверждённых команд WB создание поставки, добавление заказа и передача в доставку сразу фиксируются в локальной проекции и передаются другим устройствам через Realtime.
- Для только что созданной поставки frontend удерживает присвоенное пользователем имя, пока WB временно возвращает пустое имя; `WB-GI-…` остаётся только в колонке QR-кода и больше не подменяет заголовок поставки.
- Незавершённые переходы защищены от eventual consistency WB и от старой полной сверки, завершившейся позже. Полная и быстрая сверки используют независимые каналы.
- Миграция `supabase/patch_fbs_fast_sync_v2.sql` сначала успешно проверена в production-транзакции с `ROLLBACK`, затем применена. Edge Function `wb-fbs` развёрнута, frontend отправлен в `origin/main` commit `060cd1b`.
- Production-проверка подтвердила три таблицы и семь обязательных функций fast-v2; включённых магазинов — `0`. Перед массовым включением остаётся включить один тестовый магазин и выполнить фактическую canary-проверку.

### Файлы

- `src/pages/FbsOrdersPage.tsx`
- `supabase/functions/wb-fbs/index.ts`
- `supabase/patch_fbs_fast_sync_v2.sql`
- `memory-bank/systemPatterns.md`
...
### Стикеры QR-кодов поставки WB + кнопка пропуска (30.04.2026, историческая версия)
- Этот блок описывает прежнее состояние. Получение/сортировка package codes через API и добавление такого PDF в стикеры больше не являются действующим способом назначения WB ШК: актуальный источник — проверенный Excel WB и registry, см. текущий фокус в начале файла.
...
## 2026-08-31 — Приёмка из каталога

- Повторное добавление варианта с тем же точным баркодом не создаёт новую строку: количество суммируется в уже существующей строке текущей стадии.
- Если несколько дублей этого баркода уже существовали до исправления, следующее добавление объединяет их в одну строку, сохраняя суммы `Заявлено`, `Принято` и брака; лишние сохранённые строки проходят штатное удаление/мягкое исключение при сохранении.
- Суммирование применяется и к сохранённым строкам, и к локальным строкам черновика; изменяется количество активной вкладки `Заявлено` либо `Принято`.
- Окно результатов каталога занимает рабочую ширину до `max-w-5xl`, имеет увеличенную высоту со скроллом, крупную карточку товара и табличные строки вариантов.
- Для каждого варианта явно показаны размер, баркод, счётчик количества `− / ввод / +` и отдельная заметная кнопка `Добавить`. Enter в поле количества выполняет то же добавление.
- Остальные способы приёмки и их стили не менялись; новых колонок или SQL для этой доработки нет.
- Основное исправление опубликовано коммитом `e99b18c`, консолидация уже существующих дублей — `15876f5`; production отдаёт итоговый asset `assets/index-CZnA46ej.js`.

## 2026-08-31 — Пакетный каталог приёмки и единый FBS-селектор магазина

- В режиме `Из каталога` строковый поиск заменён одной кнопкой `Открыть каталог`.
- Каталог открывается поверх карточки партии модальным окном `90vw × 95vh` и загружает тот же полный набор локальных товаров магазина, что страница `Товары`.
- Таблица каталога повторяет товарные колонки страницы `Товары`; варианты размера изначально скрыты и раскрываются кликом по строке товара.
- У варианта есть только поле количества. Отдельных кнопок `Добавить` и добавления по Enter нет: весь набор применяется одной нижней кнопкой `Сохранить`.
- Количества модалки являются черновиком. Отмена/закрытие их сбрасывает; сохранение добавляет выбранные варианты в черновик текущей вкладки `Заявлено` либо `Принято`, не завершая внутреннюю приёмку или pipeline-стадию.
- Перед применением все выбранные варианты группируются по точному баркоду. Затем количество суммируется с существующей строкой текущей стадии; старые дубли также консолидируются без потери заявленного, принятого количества и брака.
...
## 2026-08-31 — Единый выбор склада WB в трёх вкладках FBS

- Поле `На склад WB` вкладки `Отчёт` принято источником дизайна для складских селекторов вкладок `Заказы` и `Остатки FBS`.
- Все три вкладки используют общий компонент `FbsWarehouseSelect`: одинаковые подпись, высота, ширина, рамка, фон, шрифт и focus-состояние.
...
## 2026-08-31 — Общее количество и hover-фото в каталоге приёмки

- В модальном каталоге приёмки колонка `Себестоимость` заменена на `Кол-во`; себестоимость больше не показывается и не участвует в этом сценарии.
- Поле количества в основной строке товара назначает введённое значение сразу всем его вариантам с валидным баркодом. Например, значение `10` у пяти вариантов создаёт выбор на `50` единиц.
- После массового заполнения варианты по-прежнему можно раскрыть и скорректировать отдельно. Если их значения различаются, общее поле показывает состояние `Разное`; новое значение в нём снова перезаписывает все варианты товара.
- Сохранение не менялось: позиции группируются по точному баркоду, суммируются с текущей приёмкой и применяются к активной вкладке `Заявлено`/`Принято`.
...
## 2026-09-01 — Размеры и количество в родительской строке каталога

- Решение об общем количестве для всех размеров отменено: у размерного товара в родительской строке больше нет редактируемого количества, чтобы оператор не принял его за суммарное или распределяемое значение.
- Последняя колонка родительской таблицы называется `Размеры`. Для размерного товара она показывает точное число уникальных реальных размеров (`3`, `6` и т. п.); количество каждого варианта остаётся только в раскрытых строках `Размер / Баркод / Количество`.
- Служебные WB-значения `0`, `—`, `Без размера`, `Б/Р`, `One Size` и `Универсальный` не считаются реальным размером.
- Безразмерный товар с одним валидным баркодом не раскрывается: ячейка показывает `Без размера` и компактное поле `Кол-во`, относящееся только к этому баркоду.
- Безразмерный товар с несколькими баркодами раскрывается по отдельным баркодам и получает количество в каждой дочерней строке. Товар без баркода показывает `Нет баркода` и не допускает ввод.
...
## 2026-09-01 — Порядок подписей у безразмерного товара

- В ячейке `Размеры` безразмерного товара с одним баркодом порядок элементов изменён на `Кол-во` → `Без размера` → поле ввода.
...
## 2026-09-01 — текстовое число размеров в каталоге приёмки

- В родительской колонке `Размеры` удалены рамка и фон числового бейджа.
- Число выводится обычным текстом с корректным русским склонением: `1 размер`, `2 размера`, `5 размеров`, включая формы `11 размеров`, `21 размер`, `22 размера`.
- Раскрытие размерного товара, дочерние количества и поведение безразмерного товара не менялись.
- Supabase/SQL не затронуты. `npx tsc --noEmit`, production build (666 модулей, asset `assets/index-CAlSxUOu.js`) и `git diff --check` проходят. Функциональный commit: `d7c1d40`.

## 2026-09-01 — зелёный КИЗ только по подтверждению WB

- Зелёный бейдж `КИЗ` в FBS-заказе больше не выводится из локального статуса `fbs_marking_pairs.sent`; единственный источник — непустой `sgtin` в официальных метаданных заказа WB.
- Это одинаково обнаруживает КИЗ, добавленный через ELESTET, кабинет WB или сторонний сервис. Проверяется только бинарный факт наличия привязки к конкретному FBS-заказу.
- Read-only синхронизация метаданных охватывает все актуальные `confirm` и `complete` заказы, включая `complete + sorted`, запускается при входе на `На сборке`/`В доставке`, по кнопке `Обновить` и после отправки нашей сессии.
- Локальные пары и история сканера сохранены, сканирование/отправка/баркоды/короба не менялись. Если WB не вернул строку запрошенного заказа, сохранённый WB-флаг сбрасывается в `false`.
- SQL не затронут. Edge Function `wb-fbs` опубликована как production version 41. `npx tsc --noEmit`, production build (666 модулей, asset `assets/index-CrKGshYG.js`) и `git diff --check` проходят. Функциональный commit: `6fe1a37`.

## 2026-09-04 — официальная история FBS-поставок (опубликовано 09.09.2026)

- Подготовлена параллельная временная шкала по официальным данным WB: поставка, её заказы и отдельная попытка передачи заказа внутри конкретной поставки.
- `closedAt` поставки используется как официальное время передачи, `scanDt` — как время сканирования QR поставки; принятие конкретного заказа дополнительно подтверждается его официальным статусом.
- Лёгкая синхронизация рассчитана на серверный запуск каждые 2 минуты для всех магазинов, ночная и ручная — на полную сверку. Ручная кнопка `Обновить` теперь запускает полную сверку выбранного магазина.
- Старые `fbs_dispatch_events` и старый отчёт не удаляются. Новый отчёт использует новую попытку, а для ещё не восстановленных строк оставляет legacy fallback.
- Локально проходят production build, отдельная строгая TypeScript-проверка Edge Function и `git diff --check`.
- Production-проверка 09.09.2026 подтвердила таблицы `fbs_supplies`, `fbs_supply_orders`, `fbs_dispatch_attempts`, `fbs_sync_jobs` и RPC отчёта/синхронизации. Frontend отправлен в `origin/main` commit `060cd1b`, Edge Function `wb-fbs` развёрнута.

## 2026-09-09 — управляемый каталог FBS-сканеров (опубликовано)

- В скрытую `AdminPage` добавлен таб `Сканеры`: поиск, создание черновика, публикация, изменение профиля и безопасное архивирование без физического удаления использовавшихся моделей.
- Новый профиль хранит бренд/модель, режим `keyboard` или `web_serial`, COM-параметры, окончание пакета, инструкции, предупреждение и последовательности настроечных/восстановительных CODE128 или QR-кодов.
- Модалка КИЗ получает опубликованные модели из БД и кэширует последний успешный каталог локально. Значение `Сканер не выбран` и работа неизвестных моделей без профиля сохранены.
- Глобальный keyboard-перехват и COM-приём приостанавливаются только на время внутренних окон; после закрытия глобальный фокус снова возвращается в поле скана.
- SQL `supabase/patch_fbs_scanner_catalog.sql` создаёт каталог, неизменяемую админскую историю, безопасные RPC и переносит шесть текущих моделей. Миграция применена к production Supabase по явному подтверждению пользователя; таблицы, 6 активных моделей, UTF-8 кириллицы, новая колонка сессии, удаление старого CHECK и динамическое определение RPC проверены запросом.
- PostgREST schema cache принудительно обновлён. Production build и `git diff --check` проходят. Frontend отправлен в `origin/main` commit `060cd1b`, Edge Function `wb-fbs` развёрнута.

## 2026-09-10 — точный COM-профиль MERTECH CL-1300 и целостность КИЗ

- Каталог выбора перестроен в иерархию `бренд → точные модели`; настройка вынесена в отдельную модалку и открывается шестерёнкой конкретной модели. Выбор модели и открытие настройки являются разными действиями.
- Выбранный бренд можно свернуть; свёрнутая плашка подсвечивается фиолетовым. Раскрытие/закрытие плавное по уже используемому на странице `Товары` приёму `grid-template-rows`. Desktop-размеры: selector `40vw × 95dvh`, settings `70vw × 97dvh`.
- По фотографии клиента точно определена беспроводная модель `MERTECH CL-1300 P2D` (`CL-1300 GS BLE Dongle P2D USB Black`, артикул производителя 4893). Настройки проводной `MERTECH 2310 P2D HR SUPERLEAD` к ней не применяются.
- Для модели подготовлен и применён идемпотентный production-профиль `web_serial`: `115200 8N1`, parity `none`, flow control `none`, окончание пакета `CR`, максимум 4096 байт. Настроечные коды: беспроводной USB-COM `SX060802130.` и суффикс CR `090200.`; возврат к беспроводному USB-HID — `SX060802124.`.
- Перед настройкой оператор сканирует уникальный код сопряжения, напечатанный на комплектном USB-приёмнике. Сервис этот код не генерирует.
- Production-запись имеет `status=active`, version 3 и полную историю из трёх версий; connection/serial/packet options, команды и русские подписи сверены побайтово в UTF-8.
- В Web Serial входной `Uint8Array` преобразуется побайтово, поэтому обязательный ASCII GS `0x1D` остаётся `\u001d`; CR служит только границей пакета. Это основной путь для правильного КИЗ без влияния раскладки. HID — fallback с отдельно подтверждённым model-specific GS mapping.
- Архитектурный критерий правильности: сохранить полный GS1 payload со всеми внутренними GS по цепочке от устройства до `sgtin` WB. Видимое совпадение текста недостаточно; проверка должна сравнивать code points/hex.
- Код, production-профиль и build проверены. Остаётся физический приёмочный тест настоящим CL-1300 с реальным КИЗ, содержащим GS, и подтверждение результата официальным `sgtin` WB; до него нельзя заявлять полную аппаратную совместимость.
- Следующая проблема клиента — подстановка остатков ELESTET на WB — сознательно отложена и в эту работу не входит.

## 2026-09-10 — FBS-таблица и официальный акт приёмки WB

- Во вкладках со строками таблица ограничена шириной FBS-контейнера; горизонтально движется внутреннее содержимое, а правая колонка действий/статуса остаётся sticky. Лишнее зарезервированное место после удалённого карандаша убрано подбором ширины правой колонки по реальному содержимому.
- Отдельная колонка `Заказы` родительской поставки показывает количество её заказов; связь заказа меняется оптимистично, а подтверждённый перенос сразу увеличивает счётчик поставки назначения без F5. Фоновый sync затем сверяет результат с WB.
- На родителях `В доставке` и сгруппированных `Завершённых` добавлено скачивание официального акта приёмки WB. Документ ищется в WB Documents API как `act-income-mp-{numeric supply id}` и скачивается официальным подписанным ZIP; свой Excel ELESTET не генерирует.
- Если акт ещё не создан WB, нет XLSX, отсутствует разрешение Documents API или пришёл неожиданный формат, интерфейс показывает точную ошибку и не выдаёт фиктивный файл.

## 2026-09-11 — два состояния складской единицы FBS и ручная переприёмка

- Утверждена и реализована каноническая модель: физическая единица либо `В коробе`, либо `Привязана к заказу`. При `Взять из короба` `fulfillment_box_items.qty` сразу уменьшается на 1; статусы поставки/WB это количество повторно не уменьшают.
- Технические `reserved` и `awaiting_wb` остаются подэтапами одной активной привязки только для отчёта. Подтверждённая поштучная приёмка WB закрывает её как `consumed`; отмена не возвращает единицу автоматически.
- Для ранней отмены активная строка получила действие `Принять обратно`. Оператор выбирает короб с данным SKU либо сканирует QR любого другого действующего короба на включённом FBS-складе. RPC при необходимости создаёт товарную строку, увеличивает целевой короб и закрывает привязку как `released` ровно один раз.
- Повторная переприёмка идемпотентна. `consumed` нельзя вернуть старой привязкой: последующий физический возврат WB оформляется новой приёмкой. Если WB подтвердил приёмку после ручного `released`, автоматического количества нет, а строка получает `requires_review`.
- В разделе остатков отдельно выбираются источник ELESTET (только `wms_warehouses.fbs_enabled=true`) и получатель WB. Кнопка подставляет именно `В коробах` выбранного склада; UI показывает только `В коробах`, `Привязано к заказам`, `Сейчас WB` и новое значение.
- Старые короба без WMS-размещения и без склада партии остаются видимыми с пометкой `БЕЗ FBS-СКЛАДА`, но не выбираются для сборки. Массовая подстановка блокируется при совпадающих SKU без склада, чтобы не отправить в WB ложные нули. Склад нужно назначить явно; между Бишкеком и Москвой система не угадывает.
- Production-миграция `supabase/patch_zzzz_fbs_bound_stock.sql` применена. До перехода проверены 172 активные привязки: потерянных источников и дефицитных коробов — 0. После перехода все 172 имеют `stock_removed_at`, отрицательных `qty` — 0; `reconcile_fbs_stock_allocation` больше не меняет короб.
- Полная миграция предварительно выполнялась с `ROLLBACK`; финальные определения RPC и return signatures проверены через `pg_get_functiondef`/`pg_get_function_result`. Повторное применение идемпотентно и не списывает активные строки второй раз.
- Два production-теста на реальной структуре выполнялись внутри транзакций с `ROLLBACK`: ручная переприёмка дала `released` и дельту короба `+1`, повторный вызов оставил дельту `+1`; имитация поштучной приёмки WB дала `consumed` и дельту короба `0`. После отката production-счётчики остались `172 active / 7 released`.
- Скрытые супер-админские страницы `Инфо` и `Гайд`, канонический `memory-bank/components/fbs-orders.md` и золотое правило полноты Memory Bank обновлены. Production build проходит (673 модуля, прежние предупреждения только о крупных чанках/статическом `xlsx`). Frontend пока не commit/push.

## 2026-09-12 — фантомные FBS-поставки, склад в переносе и точные ошибки

- Установлена точная причина отказа заказа `5727161044`: выбранная `WB-GI-275897127` оставалась активной только в локальной проекции, но отсутствовала в актуальном каталоге и деталях WB. Последние складские признаки совпадали, поэтому прежний текст про возможное несовпадение склада был ложной догадкой.
- Актуальный `GET /api/v3/supplies` читается один раз целиком по курсору без несуществующего `isSupplyClosed`; `done` разделяет открытые и закрытые поставки.
- `patch_fbs_supply_availability.sql` сохраняет исторические строки, добавляет `available_on_wb`/`unavailable_since` и RPC каталоговой сверки/точечной маркировки. Исчезнувшая активная поставка скрывается, но её memberships и попытки отгрузки не удаляются.
- Официальный `404` при добавлении заказа немедленно помечает целевую поставку недоступной. Остальные ожидаемые ответы WB возвращаются структурированно и не маскируются Edge HTTP `500`.
- Модалка переноса показывает отдельную безбордерную колонку склада с официальным и внутренним названием. Верхний складской фильтр ограничивает список, а фактические заказы дополнительно защищают от смешения склада, габарита, cross-border и B2B.
- Нативный browser alert заменён отдельной понятной модалкой: причина, номера неперенесённых заказов, частичный успех и подтверждение, что заказ остался в исходной поставке/без поставки.
- Production SQL применён, `WB-GI-275897127` точечно получила `available_on_wb=false`, Edge Function `wb-fbs` опубликована как version 63 с прежним `verify_jwt=true`. Frontend и Memory Bank опубликованы в `origin/main` коммитом `8c75600`; Vercel production отдаёт совпадающий с локальной сборкой asset `assets/index-CLBodkSX.js`.
