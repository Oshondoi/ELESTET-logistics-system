...
## CURRENT FOCUS — 25.09.2026: связно согласуемая редакция 15

- Активное обсуждение `Заявки клиентов, передача партий и фактическая приёмка` находится в редакции 15: полный текст сохранён и расширен до 40 нумерованных пунктов плюс нулевой; полученные решения сразу отмечены согласованными, открытыми оставлены только три явно подписанных вопроса.
...
- `A-N` означает `Applicant`: это локальный монотонный ID компании-заявителя в клиентской базе конкретного исполнителя. Одна `C` может иметь разные `A-N` у разных исполнителей; `store_code` магазина остаётся отдельным полем.
...
- UI discussion production: компактная неподвижная шапка, внутренний scroll контента, цветовая вертикальная навигация с автопрокруткой, локальный черновик галочек и единое явное сохранение опубликованы до текущей редакции 15.
