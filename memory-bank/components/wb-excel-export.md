...
> Не путать с новой выгрузкой содержимого коробов на странице партии. Этот документ описывает логистический WB-шаблон через `trip_lines.wb_package_codes`. Новая выгрузка отдельного короба/всей фулфилмент-поставки использует общий конфигуратор: 5 обязательных + 5 дополнительных колонок, XLSX/ZIP и сортировку по `box_number`.
