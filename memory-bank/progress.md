...
- Кнопка «Из WB» в форме магазина удалена
...
- Иконки ухода, EAC
- Наборы стикеров
- **Вкладка Импорт WB**: аккордеон, фото с превью, чекбоксы (глобальный + на товар + на размер), expand-all, «Создать набор» со skip дублей
- **Вкладка Кастомная**: массовое удаление — кнопка-корзина в шапке колонки удалений, неактивна пока не выбрана строка
...
- Продакшн БД: все 18 SQL-патчей применены, RLS политики восстановлены
...
- Колонка фото (миниатюра 36×36, плейсхолдер если нет)
...
### Стикеры — баркод и PDF (завершено)
- Поле баркода в форме создания/редактирования стикера
- PDF: шрифт 21px, значения полей font-weight 500

### Страница Товары
- Заглушка «Скоро» (не реализована)

## What's Left
- Этап 5: Поиск и фильтры (Логистика)
- Страница Товары (реализация)
- Участники компании (Members)
- Мобильное приложение React Native + Expo
