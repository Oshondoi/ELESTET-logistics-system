...
- [x] Локально: append-only история успешных изменяющих Excel-загрузок с пользователем, датой, файлом, результатом и лениво загружаемым сравнением `до/после` реализована в commit `b86d7c9`.
- [x] Локально: повтор полностью совпадающего WB-файла журналируется как `Без изменений`; скачивания и восстановление прошлой версии сознательно не входят в журнал.
- [x] Локально: подготовлена серверная проверка `fulfillment_manage` и закрытие прямых обходных записей/RPC.
- [ ] Deploy истории: авторизовать Supabase CLI, применить `patch_fulfillment_excel_action_history.sql`, проверить RPC/RLS, затем push frontend. До этого commit `b86d7c9` нельзя считать production.
