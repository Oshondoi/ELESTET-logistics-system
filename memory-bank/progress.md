...
- [x] Предметная модель доведена до редакции 15: полный текст сохранён и расширен до 40 нумерованных пунктов плюс нулевой; в БД сохранены 144 состояния согласования.
...
- [x] `K` заменён на локальный ID `A-N` (`Applicant`) компании-заказчика у каждого исполнителя; `store_code` остаётся отдельным идентификатором.
...
- [ ] Ответить на три открытых вопроса редакции 15: первичный пароль по ссылке, безопасная смена ошибочного имени/email и миграция старого магазина без однозначной компании клиента.
