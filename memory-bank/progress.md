...
## 26.09.2026 — реализован третий уровень согласования `Реализовано`

- [x] `item_states` расширен значениями `implemented` и `implemented:<hash>` с обратной совместимостью для прежних `agreed` и подписей блоков.
- [x] Ручной интерфейс по-прежнему переключает только `В обсуждении / Согласовано`; у реализованной строки чекбокс отсутствует, а снять или установить реализацию через UI нельзя.
- [x] Блок со строками агрегирует статусы строк; блок без строк хранит собственный подписанный статус, который сбрасывается при изменении пояснения, но сохраняется при переименовании заголовка.
- [x] Чёрные схемы не хранят решения и наследуют полный статус родителя; реализованные блоки, строки, навигация, бейджи и полосы схем используют серые оттенки с сохранением метки происхождения.
- [x] Неизменённые и перемещённые реализованные строки сохраняют статус, изменённые требования его теряют. Перед выставлением реализации ИИ обязан проверить всю актуальную редакцию.

## 26.09.2026 — согласована новая структурная модель статусов обсуждения

- [x] Зафиксирована формула `Статус = Происхождение × Согласование` и четыре цвета сочетаний.
- [x] Согласована независимость происхождения блока, строк и чёрных схем; собственным текстом блока признаны заголовок и обычные поясняющие абзацы.
- [x] Согласованы перенос строк только между родительскими блоками, нейтральная перестановка внутри одного блока, сохранение ручного согласования и составная метка `Перемещено и изменено`.
- [x] Согласована обязательная принадлежность чёрной схемы блоку либо строке, собственное происхождение схемы и наследование согласования родителя.
- [x] Согласованы правила первой содержательной версии, удаления без переноса, перенумерации блоков, обязательной загрузки истории и приоритета явных решений владельца в чате над противоречащими старыми статусами.
- [x] Реализована утверждённая модель в `DiscussionsTab.tsx`: независимое происхождение блоков, строк и схем, перенос между родителями, наследование согласования, первая версия без ложного оранжевого состояния и обязательная загрузка истории.
- [x] Добавлена совместимость со старыми ключами `item_states`; TypeScript, production build и `git diff --check` проходят.
- [x] Frontend опубликован в `origin/main` commit `ba05232`; production asset `assets/index-CHTFiz6l.js` проверен по новым строкам и состояниям.
- [x] Reset предварительно прошёл production-тест с `ROLLBACK`, затем применён к точному ключу `public-request-invite-auth-20260926`: прежний объект удалён, текущий текст сохранён в новом объекте с новым UUID, `active`, редакцией 1, без архивных ревизий и ручных отметок.

## 26.09.2026 — production-внедрение заявок и финализация статусов обсуждения

- [x] Четырёхцветная двухосевая модель обсуждения реализована для блоков, строк, навигации, бейджей и чёрных схем: оранжевый, фиолетовый, синий и зелёный отражают сочетание происхождения текста и состояния согласования.
- [x] Согласованная редакция 28 реализована в frontend и серверной модели: `R`, отдельные `P` по магазинным вкладкам, постоянные связи магазинов, локальные `A-ID`, корректировки и версии, принятие/отклонение, копирование, приглашения, акты и счета.
- [x] `20260926090000_service_requests.sql` применена в production; структура и ограничения проверены read-only запросами.
- [x] Сквозной production smoke-test прошёл внутри транзакции с полным `ROLLBACK`; после исправления внутренней привязки стадии сценарий дошёл от черновика до принятия, связи магазина, акта и счёта, тестовых строк не осталось.
- [x] Кнопка завершения всего обсуждения переименована в `Завершить редакцию`; команда владельца `закрыть редакцию` имеет тот же смысл. Текущая редакция при переименовании не завершалась.
- [x] Production build, SQL parser и `git diff --check` проходят.
- [x] Изменения до commit `ab92cab` отправлены в `origin/main`; Vercel production проверен по asset `assets/index-C0b2uXCj.js` с новой кнопкой `Завершить редакцию`.

## 25.09.2026 — связная редакция обсуждения и компактный review UI

- [x] Предметная модель доведена до редакции 17: полный текст сохранён, 40 нумерованных пунктов плюс нулевой; в БД сохранены 177 состояний согласования.
- [x] Нулевой пункт согласован целиком: статус сохранён для самого пункта и всех 20 его строк.
- [x] Зафиксировано: черновик сразу получает `R`, `P` создаётся при отправке, до принятия исполнителем заявитель может заменить `P` в той же `R`, а после принятия операционное управление переходит исполнителю.
- [x] Черновое состояние сохраняется в БД без архивной истории; подтверждённые корректировки обновляют `Заявлено`, сохраняют версии и не переписывают `Принято`.
- [x] `K` заменён на `A-N` (`Applicant`) магазина-заказчика; счётчик ведётся отдельно внутри каждой родительской компании по `stores.account_id`, а `store_code` остаётся отдельным идентификатором.
- [x] Зафиксирован первый доступ: исполнитель создаёт ссылку на 30 дней, заявитель сам задаёт пароль до открытия формы, а preliminary `U/C` и email появляются при первом подтверждении заявки.
- [x] Согласована смена имени/email без администратора: повторный пароль, подтверждение нового адреса кодом, уведомление старого адреса и сохранение всех ID.
- [x] Согласовано отображение контакта: полная почта и её статус — в детальной карточке входящей заявки и административном просмотре, маскированная почта — на восстановлении пароля, без email в общих списках.
- [x] Пункт 8 согласован: существующие магазины нумеруются внутри `stores.account_id`; восстановленный магазин с номером возвращает прежний `A-N`, а старый магазин без номера впервые получает следующий `A-N` при восстановлении. Перенос между компаниями не входит в модель.
- [x] Добавлена целевая функция копии подтверждённой заявки: новая `R`, скопированные данные без получателя и истории, новая `P` только после отправки.
- [x] Зафиксировано расширение модели стадий: обычная заявка активирует первую стадию исполнителя, существующая партия как заявка создаёт следующую стадию получателя; чужие стадии read-only.
- [x] Согласована, но ещё не реализована production-функция скрытия партий по компании с переключателем-глазом и возвратом.
- [x] Production UI обсуждения уплотнён: `Актуальный / Завершённые` подняты вправо в строку основных табов, шапка карточки остаётся неподвижной, прокручивается только содержимое.
- [x] Числовая навигация показывает состояние блока оранжевым/синим/зелёным, активный номер затемняется в своём цвете и автоматически прокручивается вслед за контентом.
- [x] Новые и изменённые блоки/строки синие; строки немного темнее родительского блока; чёрные схемы сохраняют фон и получают цветную левую полосу `12px`.
- [x] Галочки редактируются локальным черновиком; только явное `Сохранить` записывает итог и создаёт одну ревизию, `Отмена` не меняет БД.
- [x] Историческая редакция 9 восстановила и связно синхронизировала 38 блоков; позднее она без потери содержания развита до текущей редакции 11.
- [x] Закреплено постоянное правило связной редакции: изменение решения требует в той же версии обновить все зависимые пункты по полной цепочке сущностей, прав, процессов, аудита и реализации.

## 24.09.2026 — админское обсуждение, версии и согласование решений

- [x] На `/tz-prompts` добавлен первый таб `Обсуждение`; порядок: `Обсуждение / Задачи / Промпты ТЗ`.
- [x] Реализованы один актуальный ответ, завершённый архив, редактирование без дублей, `Завершить редакцию`, `Вернуть в актуальные` и автоматическая история ревизий.
- [x] Production Supabase содержит `tz_discussions`, `tz_discussion_revisions`, partial unique index одного активного ответа, superadmin RLS и архивирующий триггер.
- [x] Блоки и строки имеют сохраняемые состояния `В обсуждении / Согласовано`; нажатие блока меняет все строки, строка пересчитывает блок.
- [x] Новые блоки/строки текущей содержательной редакции выделяются светло-синим цветом Excel-действий; зелёный статус согласования имеет приоритет, метка новизны сохраняется.
- [x] Модалка истории зафиксирована на `90vh × 80vw`; редакции, цифровая навигация и контент находятся в трёх отдельных колонках.
- [x] В истории работают цифры без названий, синий активный пункт, плавный переход, scroll-spy и параллельная автопрокрутка цифровой колонки.
- [x] На актуальной карточке добавлена такая же sticky-навигация с автоматическим активным пунктом и прокруткой списка цифр.
- [x] Начальный архитектурный ответ записан в БД; revision 4 дополнен режимом приёмки уже готовых коробов. Целевая предметная модель отдельно сохранена в `memory-bank/components/intake-requests.md`.
- [x] Production frontend проверен после commit `d9ebe9b`; сборка проходит, остаются только прежние Vite warnings.
- [ ] В начале следующего продолжения предметного обсуждения перечитать актуальный `tz_discussions.item_states`: клики на сайте сохраняются, но не доставляются ИИ автоматически.

## 24.09.2026 — FBO Excel, WB ШК коробов и история действий

- [x] Production: исходный/сокращённый/гибридный однолистный Excel WB принимается по двум заголовкам; остальные колонки игнорируются, полностью пустые строки не расходуют нумерацию.
- [x] Production: API-порядок упаковок больше не назначает ШК физическим коробам. Excel WB является временным эталоном: заполненные пары последовательно соответствуют коробам №1, №2 и далее.
- [x] Production: предпросмотр показывает `Сейчас → Станет`, недостающие и лишние короба; любое несовпадение полного набора блокирует атомарное применение.
- [x] Production: проверенная пара WB сохраняется в registry по номеру короба, переживает удаление строки короба и возвращается при повторном создании того же номера.
- [x] Production: единая Excel-модалка содержит `Содержимое коробов`, `Загрузить WB ШК`, `Экспорт`; системный и WB-экспорт разделены без удаления доступных режимов из UI.
- [x] Production: WB-экспорт формирует исходный пятиколоночный однолистный шаблон WB; системный экспорт сохраняет `По коробам / По баркодам / Оба` и выбор дополнительных колонок.
- [x] Production: sync с WB сохраняет склад, дату, тип и другие сведения поставки, но не получает/не переназначает WB ШК коробов.
- [x] Production: Excel-модалка имеет стабильную высоту `90vh`, внутренний scroll и закреплённую нижнюю область действий; правило распространено на все модалки проекта.
- [x] Production: append-only история успешных изменяющих Excel-загрузок с пользователем, датой, файлом, результатом и лениво загружаемым сравнением `до/после` реализована в commit `b86d7c9`.
- [x] Production: повтор полностью совпадающего WB-файла журналируется как `Без изменений`; скачивания и восстановление прошлой версии сознательно не входят в журнал.
- [x] Production: серверная проверка `fulfillment_manage` и закрытие прямых обходных записей/RPC применены миграцией `patch_fulfillment_excel_action_history.sql` до frontend-публикации.
- [ ] Получить официальный ответ WB о приоритете интерфейса/Excel/API. До ответа автоматически менять эталон Excel запрещено.

## 17–22.09.2026 — КИЗ fulfillment, трансгран и СПОТ

- [x] В `origin/main`: физическая приёмка КИЗ в fulfillment связывает конкретный короб, товарный баркод и исходную GS1-последовательность; доступны черновик, подтверждение, просмотр, пересвязка, удаление и immutable audit.
- [x] В `origin/main`: настройка аппаратного сканера разделена по устройствам и используется и в FBS, и во fulfillment; публичное название прямого режима — `Режим ELESTET`.
- [x] В `origin/main`: FBO и FBS создают разные источники трансграна, snapshot подтверждённых КИЗ и отдельную историю; реализованы диагностика, отправка, синхронизация и отмена Teksher-операции.
- [x] В `origin/main`: поля СПОТ отделены от операции Teksher; сохраняются канал, перевозчик, транспорт, QR, декларации и независимый статус.
- [x] В `origin/main`: товарная группа операции эмиссии Teksher сохраняется серверно и повторно используется при утилизации вместо опасного fallback на `lp`.
- [ ] Перед следующим backend-изменением трансграна отдельно сверить production SQL/Edge Function с версиями в `origin/main`: этот Memory Bank-аудит проверил репозиторий, но не выполнял внешнюю production-проверку.

## 12.09.2026 — единый селектор магазина в «Стикеры и КИЗы» и «Товары»

- [x] Импорт WB на табе `Стикеры` использует общий `FbsStoreSelect` страницы `FBS Заказы`.
- [x] Таб `КИЗы` использует тот же общий селектор без прежнего кастомного dropdown и цветных точек подключения.
- [x] Страница `Товары`, вкладка `Импорт WB`, использует тот же общий селектор; список по-прежнему ограничен магазинами с WB API-ключом, вкладка `Браки` не изменена.
- [x] Во всех трёх местах совпадают подпись, native-select, размеры, рамка, фокус, placeholder и формат `Юридическое название — Название магазина`; прежняя бизнес-фильтрация списков сохранена.
- [x] Кнопка `+ Поставка` на этапе `Короба` переименована в `Создать` без изменения обработчика.
- [x] TypeScript и production build проходят (673 модуля); остаются только прежние предупреждения Vite.

## 12.09.2026 — раздельное назначение fulfillment-поставок FBO/FBS

- [x] На этапе `Короба` добавлен переключатель `FBO склады / Свой склад` строго между выбором склада и кнопкой `Создать`; внутренние значения `fbo / fbs` и логика списков не менялись.
- [x] FBO-режим ограничен системными складами WB; FBS-режим ограничен внутренними складами текущего исполнителя стадии с включённым `fbs_enabled`.
- [x] При смене режима старый выбор и поиск очищаются; пустой FBS-список объясняет, что нет склада с включённым режимом FBS.
- [x] Тип назначения и точный UUID соответствующего справочника сохраняются у `fulfillment_supplies`; название остаётся историческим снимком.
- [x] Production-миграция применена без точечной правки клиентских строк: 143 существующие поставки получили совместимый FBO-default, некорректных строк — 0.
- [x] В Memory Bank закреплён запрет на ручные исправления конкретных production-данных без прямой команды владельца; штатное исправление обязано быть системным и работать для всех одинаковых случаев.
- [x] TypeScript и production build проходят (673 модуля); остаются только прежние предупреждения Vite о размере чанков и статическом импорте `xlsx`.
...
## 26.08.2026 — ручное управление остатками FBS на WB (исходная версия; модель заменена 11.09.2026)

- [x] Добавлен раздел `Остатки FBS` внутри страницы FBS с общим выбором магазина и отдельным выбором склада продавца WB.
- [x] Каталог строится по вариантам товара `chrtId`; старые колонки физического/резерва/свободного позднее заменены каноническими `В коробах` и `Привязано к заказам`.
- [x] Реализованы поиск и фильтры: все, изменённые, расхождения с ELESTET, нулевые WB и ошибки.
- [x] Реализованы ручной ввод, подстановка остатка ELESTET без автоматической отправки, подтверждение и предупреждение о нулях; актуальный источник после 11.09.2026 — только `В коробах` выбранного внутреннего FBS-склада.
...
> Получение и сортировка package codes ниже описывают прежний процесс. Сейчас API не назначает WB ШК физическим коробам; действующий источник — проверенный Excel WB и registry.
...
### WB Excel шаблоны для поставки FBW (09.05.2026, историческая версия)
Полная документация: `memory-bank/components/wb-excel-export.md`

> Этот блок фиксирует состояние на 09.05.2026 и не является действующей архитектурой. С 23.09.2026 источник сопоставления коробов — проверенный Excel WB и registry; API-sync package codes отключён. Актуальные правила находятся в начале этого файла и в профильном документе.
...
- На тот момент ШК брались из `trip_lines.wb_package_codes`, заполненного API-синком; это правило отменено и сохранено здесь только как история изменения системы.
...
- Edge function `wb-supply` тогда сохраняла `wb_package_codes`; действующая production-версия больше не назначает коды коробам через API.
...
- [x] Уже существующие дубли того же баркода консолидируются при следующем добавлении без потери заявленного, принятого количества и брака.
...
- [x] TypeScript, production build и `git diff --check` прошли; commits `e99b18c` и `15876f5` опубликованы в `main`/`master`, основной домен отдаёт итоговый `assets/index-CZnA46ej.js`.

## Опубликовано 31.08.2026 — модальный каталог приёмки

- [x] Inline-поиск заменён кнопкой `Открыть каталог`.
- [x] Добавлена модалка ровно `90vw × 95vh` с полным списком товаров магазина и поиском по названию, артикулам и баркодам.
- [x] Товарная таблица повторяет страницу `Товары`; размеры скрыты по умолчанию и раскрываются по строке товара.
- [x] Удалены построчные кнопки `Добавить`: оператор вводит количества у нужных вариантов и применяет всё одной кнопкой `Сохранить` в футере.
- [x] Пакет сохраняется в черновик активной вкладки `Заявлено`/`Принято`, а одинаковые баркоды суммируются между собой и с существующей строкой приёмки.
- [x] Режим корректировки после завершения приёмки сохранён; завершение этапа и завершение стадии этой модалкой не выполняются.
- [x] FBS-селекторы магазина в заказах/остатках и отчёте используют один общий компонент и один формат двух названий.
- [x] `npx tsc -b --pretty false`, `npm run build` (665 модулей) и `git diff --check` проходят.
- [x] Commit `1dfd77d` отправлен в `origin/main` и `origin/master`; Vercel production отвечает HTTP `200` и отдаёт asset `assets/index-hy55yNrm.js`.

## Опубликовано 31.08.2026 — складские селекторы FBS

- [x] Дизайн `На склад WB` из `Отчёта` перенесён в соседние вкладки `Заказы` и `Остатки FBS`.
- [x] Добавлен общий `FbsWarehouseSelect`, которым теперь пользуются все три вкладки.
- [x] Наборы вариантов и логика каждой вкладки сохранены раздельно; менялось только поле выбора.
...
## Опубликовано 31.08.2026 — количество по всему товару в каталоге приёмки

- [x] `Себестоимость` в основной строке товара заменена полем `Кол-во`.
- [x] Общее количество одним вводом назначается каждому размеру/баркоду товара.
- [x] Индивидуальные поля вариантов сохранены для точечной корректировки; смешанные значения обозначаются как `Разное`.
- [x] Hover-фото переиспользует компонент страницы `Товары`, а его портал отображается выше модального окна.
- [x] Сохранены группировка по точному баркоду, суммирование дублей и разделение `Заявлено`/`Принято`.
- [x] Изменений БД нет; TypeScript, production build (666 модулей) и `git diff --check` проходят.
- [x] Функциональный commit: `c1604d3`.
- [x] Commits отправлены в `origin/main` и `origin/master`; Vercel production отдаёт проверенный asset `assets/index-CfWcr4Qd.js`.

## Опубликовано 01.09.2026 — понятное количество по вариантам каталога

- [x] Общее редактируемое количество у размерного товара удалено.
- [x] Родительская колонка переименована в `Размеры` и показывает точное число уникальных реальных размеров.
- [x] Количество размерного товара вводится только у раскрытых строк размера/баркода.
- [x] Безразмерный товар с одним баркодом показывает `Без размера` и ввод количества непосредственно в родительской строке.
- [x] Безразмерный товар с несколькими баркодами раскрывается по баркодам; без баркода отображается явное предупреждение.
- [x] Товары без массива размеров больше не теряют дополнительные верхнеуровневые баркоды.
- [x] Supabase/SQL не менялись; TypeScript, production build (666 модулей) и `git diff --check` проходят.
- [x] Функциональный commit: `be01131`.
- [x] Commits отправлены в `origin/main` и `origin/master`; Vercel production отдаёт проверенный asset `assets/index-C2N0p_Ta.js`.

## Опубликовано 01.09.2026 — порядок безразмерного количества

- [x] В родительской строке безразмерного товара подписи расположены как `Кол-во` → `Без размера` → поле ввода.
- [x] Логика количества, баркодов и сохранения не изменялась.
...
- [x] Для любого заказа `supplierStatus = complete`, включая `wbStatus = sorted`, показывается точное сообщение без технических кодов: заказ уже `В доставке`, а WB принимает КИЗ только `На сборке`.
...
## Опубликовано 01.09.2026 — понятный отказ для любого `complete`

- [x] При сканировании QR любой заказ `supplierStatus = complete`, включая `wbStatus = sorted`, сразу останавливается с сообщением `Заказ уже передан «В доставку»`.
- [x] Технические значения `complete`, `sorted` и другие `wbStatus` пользователю не показываются.
- [x] Одинаковое правило действует в клиентской диагностике, RPC QR/КИЗ и финальной серверной проверке.
- [x] Production RPC проверены, Edge Function `wb-fbs` обновлена до version 40; RPC товарного баркода не менялась.
- [x] TypeScript, production build (666 модулей, asset `assets/index-CdkUuFZb.js`) и `git diff --check` проходят; функциональный commit `d4da51b`.

## Опубликовано 01.09.2026 — текстовое число размеров

- [x] Числовой бейдж в родительской колонке `Размеры` заменён обычным текстом без рамки и фона.
- [x] Добавлено правильное русское склонение: `1 размер`, `2 размера`, `5 размеров`, `11 размеров`, `21 размер`.
- [x] Логика раскрытия, ввода количества и безразмерных товаров не изменялась; Supabase/SQL не затронуты.
- [x] TypeScript, production build (666 модулей, asset `assets/index-CAlSxUOu.js`) и `git diff --check` проходят; функциональный commit `d7c1d40`.

## Опубликовано 01.09.2026 — подтверждение КИЗ непосредственно из WB

- [x] Зелёный FBS-бейдж `КИЗ` зависит только от непустого `sgtin` в метаданных конкретного заказа WB; локальный статус пары исключён из расчёта.
- [x] КИЗ, добавленный в кабинете WB или стороннем сервисе, определяется так же, как добавленный через ELESTET.
- [x] Проверка охватывает актуальные `confirm`/`complete` заказы при открытии рабочих вкладок, ручном обновлении и после отправки КИЗ; `complete + sorted` включён.
- [x] Сканер, локальная история, товарные баркоды и короба не изменялись; SQL не затронут.
- [x] Edge Function `wb-fbs` опубликована как version 41; TypeScript, production build (666 модулей, asset `assets/index-CrKGshYG.js`) и `git diff --check` проходят. Функциональный commit: `6fe1a37`.

## Опубликовано 09.09.2026 — официальная временная шкала FBS

- [x] Подготовлены таблицы `fbs_supplies`, `fbs_supply_orders`, `fbs_dispatch_attempts`, `fbs_sync_jobs` без удаления старых данных.
- [x] Подготовлены атомарные RPC для лёгкой, полной и поставочной синхронизации, dual-write старого и нового журналов и новый отчёт с legacy fallback.
- [x] `wb-fbs` поддерживает incremental/full sync, историю окнами по 30 дней, состав всех поставок и системный sync всех подключённых магазинов.
- [x] Кнопка `Обновить` запускает полную сверку, а двухминутный клиентский режим остаётся лёгким.
- [x] Подготовлен серверный cron: каждые 2 минуты incremental, раз в ночь full.
- [x] Production build, строгая TypeScript-проверка Edge Function и `git diff --check` проходят.
- [x] SQL применён к production, Vault/серверная конфигурация настроена, Edge Function `wb-fbs` развёрнута.
- [x] Production-проверка подтвердила четыре новые таблицы и RPC синхронизации/отчёта; frontend опубликован в `origin/main` commit `060cd1b`.
- [x] Старый журнал и отчёт сохранены; новый отчёт использует официальную попытку поставки и legacy fallback для ещё не восстановленных строк.

## 10.09.2026 — MERTECH CL-1300 P2D и правильный приём КИЗ

- [x] Пользовательский каталог сгруппирован по брендам; модели раскрываются плавно, выбранный бренд можно свернуть и увидеть по фиолетовой плашке.
- [x] Выбор модели отделён от настройки: у каждой модели есть шестерёнка отдельной settings-модалки. Desktop selector — `40vw × 95dvh`, settings — `70vw × 97dvh`.
- [x] Клиентский сканер идентифицирован как отдельная беспроводная модель `MERTECH CL-1300 P2D`, без переноса настроек проводной MERTECH 2310.
- [x] Для точной модели подготовлен и применён production-профиль `web_serial`: `115200 8N1`, без parity/flow control, terminator `CR`, максимум 4096 байт.
- [x] В профиль внесены официальные команды беспроводного USB-COM `SX060802130.`, суффикса CR `090200.` и возврата в беспроводной USB-HID `SX060802124.`; уникальный pairing barcode остаётся кодом физического dongle.
- [x] Production-профиль активен, version 3; параметры, команды и UTF-8 подписи проверены побайтово, история версий сохранена.
- [x] Web Serial reader сохраняет соответствие каждого входного byte его code unit, поэтому `0x1D` принимается как `\u001d`, а настроенный CR удаляется только как граница пакета.
- [x] В Memory Bank закреплён общий инвариант: правильный КИЗ — полный GS1 payload с сохранёнными внутренними ASCII GS; визуального совпадения строки недостаточно, проверка выполняется по code points/hex.
- [x] COM/Web Serial закреплён как основной транспорт маркировки для официально поддерживающей его точной модели; HID — только проверенный model-specific fallback.
- [x] Локальный production build проходит; остаются только прежние предупреждения Vite о размере чанков/`xlsx`.
- [ ] Провести физический end-to-end тест настоящим CL-1300: pairing → USB-COM → выбор порта → реальный КИЗ с внутренним GS → локальная проверка → привязка к заказу `На сборке` → официальный непустой `sgtin` WB.
- [ ] После физического теста отдельно зафиксировать фактические code points/hex входа и только тогда отметить аппаратную совместимость полностью подтверждённой.

## 10.09.2026 — FBS-таблица и акт приёмки

- [x] В табличных вкладках внешний FBS-контейнер ограничен доступной шириной, внутренняя область получила собственную прокрутку, правая колонка действий/статуса остаётся sticky.
- [x] Ширина sticky-колонки родителя и дочерки привязана к фактическому содержимому; лишний резерв после удалённого карандаша убран, корзина пустой поставки сохранена, а у непустой остаётся disabled.
- [x] Количество заказов родительской поставки вынесено в отдельную колонку; подтверждённый перенос сразу увеличивает счётчик поставки назначения без F5, после чего фоновый sync сверяет результат с WB.
- [x] Родители на `В доставке` и сгруппированных `Завершённых` получили скачивание официального акта приёмки через WB Documents API.
- [x] Для `WB-GI-N` используется точный документ `act-income-mp-N`; ELESTET скачивает подписанный WB ZIP и не генерирует собственный Excel.
- [x] Состояния `акт ещё не сформирован`, `XLSX недоступен`, ошибка прав и неожиданный формат обрабатываются без фиктивного скачивания.

## 11.09.2026 — двухсостоянийный FBS-остаток и ручной возврат

- [x] Пользовательская модель упрощена до `В коробах` и `Привязано к заказам`; старые колонки физического, резерва, свободного и расчётного остатка удалены из рабочего UI.
- [x] Взятие товара атомарно уменьшает исходный короб; передача поставки и приёмка WB больше не списывают эту единицу второй раз.
- [x] Отмена сохраняет активную привязку до фактического действия `Принять обратно`.
- [x] Переприёмка поддерживает выбор существующего короба товара и QR любого целевого короба включённого FBS-склада, создаёт позицию при необходимости и защищена от повторного увеличения.
- [x] Позднее подтверждение WB после ручного возврата и legacy-аномалия `consumed` без снятия помечаются `requires_review` без скрытой корректировки количества.
- [x] В остатках появились независимые селекторы `Со склада ELESTET` и `На склад WB`; подставляется только фактическое количество в коробах выбранного внутреннего склада.
- [x] Короба без назначенного внутреннего склада остаются видны, но не используются для FBS. При совпадающих нераспределённых SKU массовая подстановка блокируется.
- [x] Production-миграция прошла предварительный `ROLLBACK`, затем применена. 172/172 активных привязки преобразованы, потерянных источников — 0, дефицитных и отрицательных коробов — 0.
- [x] Транзакционные production-тесты с `ROLLBACK` подтвердили: ручная приёмка `+1`, её повтор без второго увеличения; приёмка WB закрывает `consumed` с дельтой короба `0`.
- [x] Скрытые `Инфо`/`Гайд` и Memory Bank обновлены с условиями, переходами, исключениями и правилами аудита.
- [x] `npm run build` проходит (673 модуля); остаются только прежние предупреждения Vite о размере чанков и `xlsx`.
- [x] Frontend опубликован в `origin/main` commit `30628ee`; production Supabase обновлён.

## 11.09.2026 — глобальное включение fast-sync v2

- [x] По прямой команде владельца fast-v2 назначен всем существующим магазинам.
- [x] Для будущих магазинов отсутствие отдельной строки настройки означает включённый fast-v2.
- [x] Явное `fast_sync_v2_enabled=false` сохранено как аварийный откат одного магазина; при недоступности таблицы настроек Edge Function также использует legacy.
- [x] Быстрый order-lane, независимый фоновый supply-lane и полная legacy-сверка не изменялись; меняется только глобальный режим по умолчанию.
- [x] SQL предварительно проверен в production-транзакции с `ROLLBACK`, затем применён: магазинов `102`, настроек `102`, включено `102`, выключено `0`, DB default — `true`.
- [x] Edge Function `wb-fbs` развёрнута как production version 62; production build (673 модуля) и `git diff --check` проходят.

## 12.09.2026 — надёжный перенос в существующую FBS-поставку

- [x] Подтверждено по live WB API: заказ `5727161044` активен в исходной поставке, а выбранная `WB-GI-275897127` больше не существует в каталоге WB; складские признаки не были причиной отказа.
- [x] Удалён несуществующий параметр `isSupplyClosed`; поставки загружаются единым официальным курсорным списком и делятся по `done`.
- [x] Добавлены `available_on_wb`, `unavailable_since`, безопасная каталоговая сверка и точечное скрытие по `404` без удаления исторических связей.
- [x] Production-миграция `patch_fbs_supply_availability.sql` применена; фантомная `WB-GI-275897127` точечно скрыта, остальные поставки вручную не менялись.
- [x] Ожидаемые отказы WB возвращаются структурированно; общий браузерный alert и догадки о причине заменены предметной модалкой с состоянием заказа.
- [x] В выборе существующей поставки добавлена колонка склада с двумя названиями, строки сделаны без бордеров, верхний фильтр склада и проверка совместимости заказов ограничивают кандидатов.
- [x] Edge Function `wb-fbs` развёрнута как version 63 без изменения `verify_jwt=true`.
- [x] Production build проходит (673 модуля); остаются прежние предупреждения о крупных чанках и статическом `xlsx`.
- [x] Frontend и Memory Bank отправлены в `origin/main` коммитом `8c75600`; Vercel production отдаёт проверенный asset `assets/index-CLBodkSX.js`.
