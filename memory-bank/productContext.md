...
## FBO/FBW Excel and box UX

- Оператор сначала формирует короба и их содержимое в ELESTET, затем отдельно связывает их с пустыми коробами поставки WB. WB не является источником состава коробов.
- Одна зелёная Excel-иконка поставки открывает стабильное окно с вкладками `Содержимое коробов`, `Загрузить WB ШК` и `Экспорт`; подвал окна всегда закреплён снизу.
- Импорт WB принимает исходный, сокращённый или гибридный однолистный Excel по точным названиям двух нужных колонок. Все остальные колонки игнорируются; пустые строки не занимают номер короба.
- Перед записью оператор видит полное сравнение текущих и новых WB ШК. Лишняя пара или короб без пары показаны отдельно и блокируют подтверждение, поэтому ошибочное частичное сопоставление невозможно.
- Экспорт с системным ШК сохраняет режимы `По коробам / По баркодам / Оба` и дополнительные колонки. Экспорт с WB ШК создаёт готовый пятиколоночный однолистный шаблон WB только `По коробам`; недоступные системные режимы остаются видимыми с подсказкой `Доступно для системных ШК`.
- Синхронизация FBO с WB обновляет безопасные сведения поставки, но не получает и не переназначает ШК коробов WB.
...
### Первый реальный клиент — индивидуальное условие подписки (зафиксировано 24.09.2026)

- Рабочее название клиента — `Ак Логистик`; точное официальное написание ещё нужно подтвердить, поэтому это название нельзя автоматически использовать как юридическое наименование в документах.
