...
- K-места нумеруются слева направо и сверху вниз. В паллетоместе они отображаются физической сеткой, а не одной горизонтальной строкой. Модалка показывает только выбранную на основной странице сторону стеллажа.
...
- В текущей модели зона (`wms_zones`) является целым стеллажом: столбцы A–Z — паллетоместа, строки 1–N — ярусы; адрес паллетоместа выглядит как `B3`.
...
- Стороны стеллажа (`wms_zone_sides`) настраиваются пользователем, но их может быть не больше двух. Для каждой стороны задаются название, ширина и высота сетки; по умолчанию создаются `Сторона 1` и `Сторона 2` с сеткой `2×4`, по 8 мест. Новый стеллаж по умолчанию имеет 3 яруса.
