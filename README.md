...
- У каждого короба есть сохранённый в БД уникальный `barcode`; старые поставки и короба получили номера и ШК миграцией `patch_fulfillment_box_barcodes.sql`. Реестры выданных номеров поставок и коробов запрещают повторно использовать код даже после удаления записи.
