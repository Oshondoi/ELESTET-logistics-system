...
- Стороны стеллажа (`wms_zone_sides`) настраиваются пользователем, но их может быть не больше двух. Для каждой стороны задаются название, ширина и высота сетки; по умолчанию создаются лицевая и задняя стороны `2×4`, по 8 мест.
- Операционный адрес короба: `Название стороны · B3-K5`, например `Лицевая сторона · B3-K5`; склад и стеллаж задают вышестоящий контекст. Внутри системы стороны имеют стабильные коды `S1`, `S2`, поэтому переименование не ломает адресные связи.
- Паллетоместа (`wms_cells`) имеют статусы свободно/занято/резерв/заглушено. Позиции (`wms_cell_items`) содержат товар либо один физический короб; короб привязан к `side_id + slot_number` и имеет уникальный ШК, отображаемый как QR. В модалке паллетоместа RPC `move_or_swap_wms_box` переносит выбранный короб между K-местами/сторонами этого паллетоместа или атомарно меняет два занятых короба местами.
