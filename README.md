...
- **Многоэтапный процесс** (каждый этап можно включить/отключить при создании партии):
  ...
  2. **ОТК** — ввод кол-ва после контроля качества (исходник = кол-во из Приёмки)
  3. **Маркировка** — ввод кол-ва после маркировки (исходник = ОТК)
...
**Файлы:**
- `supabase/patch_fulfillment.sql` — 4 таблицы: `fulfillment_settings`, `fulfillment_batches`, `fulfillment_items`, `fulfillment_stage_logs`; RLS по `account_members`; добавляет права в `roles`
- `src/services/fulfillmentService.ts` — CRUD батчей/позиций + `advanceStage` (умный переход с пропуском отключённых этапов) + `lookupProductByBarcode`
- `src/pages/FulfillmentPage.tsx` — полная страница: список партий с фильтром, CreateBatchModal, EditBatchModal, BatchDetailModal, SettingsModal
- `src/types/index.ts` — `FulfillmentBatch`, `FulfillmentItem`, `FulfillmentSettings`, `FulfillmentStage`, `FulfillmentBatchWithItems` + `fulfillment_view/manage` в `RolePermissions`

⚠️ **Требует**: запустить `supabase/patch_fulfillment.sql` в Supabase Dashboard → SQL Editor
