...
## Актуальное состояние разработки — 11.08.2026
...
- Production Supabase содержит актуальную WMS-схему: не больше двух сторон, стандартная сетка каждой стороны `2×4`, атомарное сохранение планировки и RPC переноса/обмена коробов.
...
- Локальные UI-изменения надёжной FBS-синхронизации и WMS ожидают отдельного commit/push в `main`; Vercel production отслеживает эту ветку.
- Полная production-сборка 11.08.2026 проходит успешно: 648 модулей.
...
- Паллетоместа (`wms_cells`) имеют статусы свободно/занято/резерв/заглушено. Каждая сторона двухстороннего стеллажа показывается отдельной таблицей и образует собственный адресный контекст. Позиции (`wms_cell_items`) содержат товар либо один физический короб; короб привязан к `side_id + slot_number` и имеет уникальный ШК, отображаемый как QR. В модалке паллетоместа RPC `move_or_swap_wms_box` переносит выбранный короб между K-местами/сторонами этого паллетоместа или атомарно меняет два занятых короба местами.
