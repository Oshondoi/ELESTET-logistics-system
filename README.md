...
- **Мульти-провайдер**: Claude (Sonnet/Haiku/Opus) и OpenAI (gpt-4o-mini/gpt-4o/gpt-3.5-turbo) — оба поддерживают Vision (фото из отзыва)
...
- **Системный промпт**: overlay-модалка с draft state (Отмена отменяет правки, Сохранить применяет)
- **Промпт магазина**: хранится в `stores.ai_prompt`, добавляется после системного промпта при вызове ИИ
