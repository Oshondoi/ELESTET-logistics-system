...
## Актуальное состояние разработки — 16.08.2026

### FBS-заказы
- Страница `/fbs` читает кэш из Supabase и синхронизируется с WB через Edge Function `wb-fbs`.
- Для системного владельца (`superadmin`) рядом с заголовком доступны внутренние вкладки `FBS Заказы` и `Инфо`. `Инфо` — статичная справка по статусам и поставкам WB без запуска синхронизации; для остальных пользователей второй вкладки не существует.
- Вкладки совпадают с WB: `Новые`, `На сборке`, `В доставке`, `Завершённые`, `Отменённые`, `Архив`.
- Источник вкладки — только официальная пара `supplierStatus + wbStatus`; браузер больше не хранит и не придумывает статусы заказов.
- Синхронизация загружает все страницы за 30 дней до точного `next=0`, получает статусы пакетами максимум по 1000 ID и сохраняет полный снимок одной атомарной операцией.
- WB order ID и курсоры обрабатываются как точные строки: 64-битные числа не округляются JavaScript.
- Если WB не отдал полный снимок, прежние корректные данные остаются в БД, а пользователь видит предупреждение и время последней успешной синхронизации.
- После записи сверяются количество строк снимка и журнал синхронизации. Неполные данные не принимаются молча.
- ELESTET никогда не отменяет FBS-заказы: отмена только читается из статусов WB.
...
- Принтер в строке и печать выбранных заказов открывают одинаковый выбор: информация о товаре, адрес товара, товарный штрихкод и WB-стикер. Принтер на родительской поставке дополнительно предлагает данные поставки. Комплект собирается в PDF `58×40 мм` по точным вариантам товара.
- Колонка `Склад FBS` показывает официальный пункт приёмки WB по `officeId`, а второй строкой — пользовательское имя склада продавца по `warehouseId`. Оба справочника загружаются одним вызовом Edge Function, внутри которого запросы WB выполняются параллельно.
...
- По точному баркоду FBS система одним пакетным RPC ищет товар в содержимом актуальных fulfillment-коробов компании. Для размещённого короба показывается полный WMS-адрес, для неразмещённого — партия, поставка и номер короба; если коробов несколько, доступны все совпадения и их количества.
- `Не найден` означает, что баркода нет ни в одном актуальном коробе компании. Адреса не копируются в FBS-кэш и всегда читаются из текущего WMS-размещения.
...
- Кнопка `Сканировать КИЗ` запускает быстрый режим `QR WB → КИЗ` без предварительного выбора поставки или заказа. Заказ и его поставка определяются по официальному стикеру WB из заранее подготовленного каталога.
- У каждого браузера/устройства собственная постоянная сессия: один аккаунт на нескольких устройствах не смешивает работу, завершение отправляет в WB только пары текущего устройства.
- Каждая готовая пара сохраняется в Supabase одной транзакцией. В пределах магазина БД отдельно запрещает повтор заказа WB, считанного QR WB и КИЗ; ограничения общие для всех аккаунтов и устройств компании.
- Незаконченный шаг `QR WB` арендует заказ на 2 минуты и продлевается heartbeat. После потери устройства блокировка истекает, а сохранённые пары можно забрать из прерванной сессии на другом устройстве.
- Отправка КИЗ выполняется только при завершении сессии через Edge Function. Частичный успех не теряется: принятые WB пары фиксируются, ошибочные остаются для повторной отправки или исправления.
- Подробности: `memory-bank/components/fbs-orders.md`.

### Фулфилмент и Excel коробов
- Системный ШК физического короба имеет единый формат `EL_C{компания}_P{партия}_S{поставка}_B{короб}`, например `EL_C52_P43_S41_B13`; тот же текст кодируется в QR и печатается под ним.
- Номер короба уникален только среди активных коробов одной поставки. Если короб удалили и затем вручную создали с тем же номером в той же поставке, он получает прежний QR; одновременно иметь два таких активных короба нельзя.
- `S` — постоянный сквозной номер поставки внутри компании. Он назначается один раз при создании поставки и не меняется при передаче в логистику.
- У каждого активного короба есть сохранённый в БД уникальный `barcode`; старые поставки и короба получили номера и ШК миграцией `patch_fulfillment_box_barcodes.sql`. Номер поставки после удаления не используется повторно, а удалённый номер короба можно восстановить только внутри его прежней поставки.
...
- В `ШК короба` выгружается системный код соответствующего короба; при нескольких товарных строках одного короба он повторяется в каждой строке. `Номер короба` остаётся простым числом.
...
### WMS — стеллажи и паллетоместа
- Иерархия: `Склад → Стеллаж → Паллетоместо B3 → Сторона → Место K5 → Короб → Товар`.
- У стеллажа одна или две стороны. Для каждой стороны настраивается визуальная сетка мест коробов; стандарт — `2 по ширине × 4 по высоте = 8`.
- Пользователь отдельно выбирает вертикальное направление цифр для ярусов стеллажа и K-мест каждой стороны: `1 сверху → вниз` или `1 снизу → вверх`. Внутри уровня K-места всегда идут слева направо. Адреса и QR при смене направления не меняются — меняется их положение на схеме.
- Свободное место принимает существующий короб фулфилмента. Занятый короб можно перенести в свободное место или атомарно поменять местами с другим коробом.
- Перемещение меняет только адрес: ШК, QR, название и содержимое физического короба сохраняются.
- Действие `Убрать адрес` снимает только WMS-размещение короба и при необходимости освобождает пустое паллетоместо; сам короб, его содержимое, партия и поставка остаются в системе.
- QR паллетоместа имеет формат `C{компания}_W{склад}_R{стеллаж}_F{сторона}_{адрес}`, например `C52_W1_R1_F1_A1`; QR конкретного коробоместа дополняется номером K: `C52_W1_R1_F1_A1_K6`.
- Стабильные коды сторон — `F1/F2`. Production-миграция обновляет существующие стороны без смены UUID и потери размещений; старые напечатанные WMS QR с `S1/S2` по-прежнему распознаются. Fulfillment-поставки сохраняют отдельный префикс `S` в ШК коробов.
- На странице склада сканирование работает в обоих порядках: `паллетоместо → K → короб` или `короб → паллетоместо → K`. Система никогда не выбирает первое свободное K автоматически.
- Полное паллетоместо блокирует новое заполнение; перенос и обмен выполняются только после подтверждения. Служебный QR сброса: `EL_WMS_RESET_V1`.
- QR паллетоместа и всех K выбранной стороны печатаются на этикетках `60×40 мм`.
- Стеллаж поддерживает видимые металлические стойки через равный интервал либо произвольные пользовательские границы.
- БД блокирует третью сторону, потерю занятых мест при уменьшении сетки и два короба в одном месте.
- `Свободна` и `Занята` вычисляются только по фактическому наличию коробов. Ручной `Резерв` удалён; оператор вручную включает и снимает только `Заглушена`.
- В WMS размещаются только существующие короба фулфилмента. Создать произвольный складской короб или назначить адрес товару без короба нельзя.
- Очередь `Без адреса` показывает принятые валидные короба, поиск возвращает все адреса совпадения во всех складах и стеллажах компании.
- Массовое размещение поставки распределяет короба по нескольким паллетоместам и выполняется атомарно: если свободных K-мест не хватает, не размещается ничего.
- При выборе последнего статуса логистики `Отгружен` интерфейс предупреждает об освобождении адресов. После подтверждения БД снимает адреса всех коробов поставки, сохраняя сами короба и содержимое.
- Каждое размещение, перенос, обмен, снятие адреса и автоматическое освобождение записывается в неизменяемую историю WMS.
- Инвентаризация встроена в страницу склада: ожидаемый состав фиксируется на старте, сканы отмечают найденные, ошибочные адреса, недостачи и лишние короба.
- Доступ разделён правами `wms_view`, `wms_manage`, `wms_history`, `wms_inventory`; сканер сообщает результат одновременно цветом, текстом и звуком.
- Подробности: `memory-bank/components/wms.md`.

### Состояние поставки изменений
- Production Supabase содержит миграцию `patch_fbs_reliable_sync.sql`; актуальная Edge Function `wb-fbs` задеплоена.
- Production Supabase содержит актуальную WMS-схему и операционный патч `patch_wms_operations.sql`: поиск, очередь без адреса, история, инвентаризация, вычисляемые статусы мест и автоматическое освобождение после отгрузки.
- Production Supabase содержит `patch_fbs_marking_scanner.sql`; Edge Function `wb-fbs` поддерживает каталог QR и надёжную отправку сессий КИЗ.
- Production Supabase содержит постоянную нумерацию fulfillment-поставок и уникальные ШК коробов формата `EL_C…_P…_S…_B…`; проверено 87 поставок и 1982 существующих короба без пустых значений и дублей.
- Текущие UI-изменения WMS находятся в рабочей копии и ожидают отдельной команды на commit/push в `main`; Vercel production отслеживает эту ветку.
- Полная production-сборка 15.08.2026 проходит успешно: 653 модуля.

## FBS Заказы — надёжная синхронизация (11.08.2026)

### Паттерн: Stale-While-Revalidate
Браузер читает из Supabase DB (мгновенно). WB API вызывается только при синке.

```
Открыл страницу  → читаем последний корректный снимок fbs_orders
                 → фоновый sync_orders; во время работы данные не исчезают
Нажал "Обновить" → полный снимок WB → атомарная запись → перечитываем DB
```

### Статусы WB (supplierStatus + wbStatus)
| Официальная комбинация | Таб |
|---|---|
| `new + waiting` | Новые |
| `confirm + waiting` | На сборке |
| `complete` без финального `wbStatus` | В доставке |
| `sold`, `canceled_by_client`, `declined_by_client`, `defect`, `canceled` либо `supplierStatus=cancel` | Завершённые (финальная история до 3 месяцев) |
| `declined_by_client` в актуальном снимке | Отменённые |
| финальный заказ старше 3 месяцев | Доступен для формирования отчёта в Архиве |

Бейдж вкладки **«В доставке»** не равен размеру списка: как в кабинете WB, он показывает только актуальные заказы `supplierStatus=complete` + `wbStatus=waiting` — переданные в доставку, но ещё не принятые Wildberries. Внутри вкладки по-прежнему отображаются также принятые, отсортированные и ожидающие покупателя незавершённые заказы.

Список «В доставке» сгруппирован по закрытым WB-поставкам: родительская строка содержит название, `WB-GI-…`, склад и количество заказов, а внутри раскрываются сами заказы. Метаданные родителей читаются через `GET /api/v3/supplies?isSupplyClosed=true`; закрытые поставки нельзя повторно передать в доставку или изменить переносом заказов.

На вкладках «На сборке» и «В доставке» чекбоксы поставок отделены от чекбоксов заказов внутри: выбор родителя не выбирает дочерние строки. Общий чекбокс над списком работает только с поставками.

На вкладке **«Завершённые»** по умолчанию показан плоский список заказов с понятным финальным статусом. Переключатель **«Сгруппировать по поставкам»** включает тот же родительско-дочерний аккордеон, что на вкладках «На сборке» и «В доставке»: чекбоксы поставок и вложенных заказов остаются независимыми. Кнопка **«Выгрузить в Excel»** экспортирует текущий список завершённых заказов. Отменённый заказ может одновременно учитываться в «Завершённых» как финальная история и в «Отменённых» как причина завершения — это соответствует представлению WB.

Вкладка **«Отменённые»** показывает только актуальные ранние отмены покупателя (`declined_by_client`) плоским списком, без группировки по поставкам и печати стикеров. Отмена продавцом не смешивается с этим оперативным списком.

Вкладка **«Архив»** не классифицирует пропавшие из 30-дневного API-снимка строки как отдельные заказы. Это генератор XLSX по сохранённым финальным заказам старше 3 месяцев: пользователь выбирает период, отчёт фиксирует точные ID заказов, хранится 7 дней и может быть скачан или удалён. Одновременно на магазин хранится не более 100 действующих отчётов; метаданные находятся в `fbs_archive_reports`.
...
- `GET /api/v3/orders` читается постранично до точного `next=0`; `limit` — не больше 1000.
- `POST /api/v3/orders/status` вызывается пакетами не больше 1000 ID.
- `PATCH /api/v3/supplies/{supplyId}/deliver` передаёт целую поставку в доставку.
- Отмену выполняет WB/покупатель; ELESTET только сохраняет и показывает официальный статус.

### Edge Function wb-fbs
...
`get_sticker`, `update_stocks`, `deliver_supply`.

- Повторяет временно неуспешные чтения WB (`408`, `425`, `429`, `5xx`) с ограниченным backoff.
- Сохраняет int64 order ID и cursor без округления.
- Не записывает частичный результат: RPC `apply_fbs_sync_snapshot` проверяет снимок и меняет данные в одной транзакции.
- При ошибке обновляется только текст ошибки в журнале; последний успешный снимок сохраняется.
- Параллельные синхронизации одного магазина объединяются в одну операцию.

### DB таблицы
- `fbs_orders` — кэш WB, `wb_order_id text`, `supplier_status`, `wb_system_status`, `status_synced_at`, `is_in_latest_snapshot`; `UNIQUE(store_id, wb_order_id)`.
- `fbs_sync_log` — последний успешный снимок (`last_synced_at`, `orders_count`, `status_counts`, `snapshot_from`, `error`).
- RPC `get_fbs_product_locations(account_id, barcodes[])` — пакетный read-only поиск всех коробов товара, партии/поставки и текущего WMS-адреса.
- SQL: `supabase/patch_fbs_db.sql`, `supabase/patch_fbs_reliable_sync.sql`.

## WMS Склад (Lite) — 15.08.2026

### Что реализовано
- Склады (`wms_warehouses`): название, стеллажи, `fbs_enabled`, `wb_warehouse_id`.
- При открытии страницы автоматически выбираются первый склад и его первый стеллаж. Каждая новая компания сразу получает склад `Основной склад`; существующие компании без WMS-склада также дополнены им. Единственный склад компании нельзя удалить — правило защищено интерфейсом и триггером БД.
- В текущей модели зона (`wms_zones`) является целым стеллажом: столбцы A–Z — паллетоместа, строки 1–N — ярусы; адрес паллетоместа выглядит как `B3`. Поле `row_number_direction` задаёт, расположен ярус 1 сверху или снизу.
- Вертикальные стойки стеллажа хранятся явно в `upright_after_cols`. Пользователь выбирает одинаковый интервал (по умолчанию каждые 3 места) либо расставляет стойки вручную между любыми столбцами.
- Стороны стеллажа (`wms_zone_sides`) настраиваются пользователем, но их может быть не больше двух. Для каждой стороны задаются название, ширина, высота сетки и `slot_number_direction`; по умолчанию создаются `Сторона 1` и `Сторона 2` с сеткой `2×4`, по 8 мест и направлением сверху вниз. Новый стеллаж по умолчанию имеет 3 яруса.
- Операционный адрес короба: `Название стороны · B3-K5`, например `Сторона 1 · B3-K5`; склад и стеллаж задают вышестоящий контекст. Внутри системы стороны имеют стабильные коды `F1`, `F2`, поэтому переименование не ломает адресные связи. Буква `S` остаётся только у номера fulfillment-поставки; старые уже напечатанные WMS QR с `S1/S2` сканер продолжает принимать.
- Паллетоместа (`wms_cells`) имеют вычисляемые статусы `free/occupied` и единственный ручной статус `disabled`; старый `reserved` мигрирован в `disabled`. Каждая сторона двухстороннего стеллажа показывается отдельной таблицей и образует собственный адресный контекст. Позиции (`wms_cell_items`) содержат только существующий физический короб фулфилмента; короб привязан к `side_id + slot_number` и имеет уникальный ШК, отображаемый как QR. RPC `move_or_swap_wms_box` переносит выбранный короб или атомарно меняет два занятых короба местами.
- Содержимое короба хранится в `wms_box_contents`. Старые короба сохранены без изменения и показываются как «Место не назначено», пока пользователь не выберет сторону и K-место.
- Нельзя поставить два короба в одно место, уменьшить сторону поверх занятого K-места, удалить используемую сторону или уменьшить сетку поверх существующего паллетоместа.
- Верхняя панель страницы содержит глобальный поиск, `Без адреса`, историю и инвентаризацию. Поиск не схлопывает одинаковый товар: один и тот же товар в трёх местах возвращает три адреса.
- Права WMS назначаются через страницу ролей. Просмотр, управление размещением, история и инвентаризация разделены.
...
- SQL: `supabase/patch_wms.sql`, `patch_wms_boxes.sql`, `patch_wms_disabled.sql`, `patch_wms_rack_layout.sql`, `patch_wms_scanning.sql`, `patch_wms_default_warehouse.sql`.
...
### QR-этикетки fulfillment-коробов (13.08.2026)

- Системный ШК/QR: `EL_C{company}_P{batch}_S{supply}_B{box}`.
- Печать открывается горизонтальным PDF-файлом строго `60×40 мм`, по одной странице на короб.
- Читаемая часть этикетки содержит номер короба через `№`, магазин, склад назначения, партию, поставку и один жирный чёрный ШК снизу; информационный блок выровнен влево.
- QR можно печатать для одного короба, сразу для всех коробов поставки или массово из списка партий с выбором поставок.
- Для печати достаточно существования короба; наличие товаров внутри не требуется.

### Дополнительные FBS-стикеры (17.08.2026)

- Наши страницы «Данные о поставке», «Информация о товаре», «Адрес товара» и «Штрихкод товара» используют читаемый крупный шрифт под этикетку `58×40 мм` без избыточного увеличения растра.
- Длинные строки переносятся без принудительного сжатия. Для точного варианта товара печатается одна адресная этикетка: лучший доступный адрес и количество остальных коробов с этим товаром.
- Официальное изображение «Стикер WB» добавляется в PDF без изменения и не проходит через наш генератор дополнительных этикеток.
- Для быстрой сборки больших PDF текстовые страницы кодируются как JPEG высокого качества; страницы с товарным штрихкодом остаются lossless PNG и переиспользуются для одинаковых вариантов товара.

### Стабильное состояние биллинга при загрузке (17.08.2026)

- Баннер окончания триала, режим чтения и тарифные ограничения рассчитываются только после загрузки активного override и платформенной роли.
- Ответ override привязан к ID компании; запоздавший ответ от ранее выбранной компании игнорируется.
- Фоновое минутное обновление сохраняет последний подтверждённый статус и не вызывает мигание интерфейса.
