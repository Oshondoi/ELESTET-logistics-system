...
## Актуальное состояние разработки — 09.08.2026

### FBS-заказы
- Страница `/fbs` читает кэш из Supabase и синхронизируется с WB через Edge Function `wb-fbs`.
- Таб «На сборке» сгруппирован по поставкам WB: поставка — родитель, заказы — дочерние строки аккордеона; видны и пустые поставки.
- Заказ можно печатать и переносить в другую существующую или новую поставку; перенос обозначен двумя встречными стрелками.
- Передача в доставку выполняется только на уровне всей родительской поставки. На родителе используется одиночная стрелка с подсказкой «Передать в доставку»; кнопок отгрузки у отдельных заказов и в массовой панели нет.
- Массовая панель появляется снизу по центру после выбора заказов, слегка затемняет фон и содержит счётчик, печать, перенос, лист подбора и крестик сброса выбора.
- Лист подбора доступен в PDF и XLSX; включает фото, бренд, название, размер, цвет, артикул продавца, WB-стикер и баркод.
- Баркод берётся из `skus` ответа FBS WB. По баркоду/`chrtId` находится конкретный размер в `products.sizes`; фото, название, бренд и артикул продавца берутся из `products`.
- Если размер не найден, интерфейс не подставляет случайный вариант: показывает «Размер не определён» и предлагает синхронизацию товаров.
- Имя новой поставки по умолчанию: `Название магазина ДД.ММ.ГГГГ`.
- Подробности: `memory-bank/components/fbs-orders.md`.

### Фулфилмент и Excel коробов
- Внутри короба один баркод занимает одну строку, повторный скан увеличивает количество; редактирование подтверждается галочкой, полное удаление — корзиной с подтверждением.
- Режим «Авто» запоминается в `localStorage`.
- Excel отдельного короба, всей поставки и поставок из списка партий используют общий выбор колонок.
- Обязательные колонки: `Баркод товара`, `Кол-во товаров`, `ШК короба`, `Срок годности`, `Номер короба`; дополнительные: артикул WB, артикул продавца, название, цвет, размер.
- Для нескольких поставок создаётся ZIP: `ALL` для всех или `CUSTOM` для выборочных; пустые выбранные поставки блокируют продолжение.
- Коробы в общей выгрузке сортируются по `box_number` по возрастанию.

### «Промпты ТЗ» → задачи
- `/tz-prompts` имеет два таба: первым «Задачи», вторым «Промпты ТЗ»; последний таб запоминается в `localStorage`.
- Задачи добавляются по одной, отображаются компактным вертикальным списком, отмечаются выполненными и удаляются с подтверждением.
- Выполненная задача становится серой и зачёркнутой. Доступ ограничен `superadmin` через RLS таблицы `tz_tasks`.
- Заголовок «Новая задача» и подсказка по Enter удалены; Enter по-прежнему сохраняет, Shift+Enter создаёт новую строку.
- Между полем и кнопкой «Добавить» есть иконка будущей сортировки; сейчас это визуальная заглушка без бизнес-логики.
- Страница не добавляет собственные внешние padding/scroll поверх контейнера `App.tsx`, поэтому выровнена с другими админскими страницами.
- SQL `supabase/patch_tz_tasks.sql` и сид `supabase/seed_tz_tasks_20260809.sql` применены к production Supabase.
- Подробности: `memory-bank/components/tz-tasks.md`.

### Состояние поставки изменений
- Код FBS, задач, SQL и документация включены в ветку `main`; Vercel production отслеживает push этой ветки.
- Полный `npm run build` перед публикацией 09.08.2026 прошёл успешно.
...
- Склад → `fulfillment_items.notes`; при сохранении → атомарно создаёт/синхронизирует поставку + N коробов
- Количество можно менять на Приёмке: увеличение добавляет номера, уменьшение удаляет последние только если они пустые
- Готовые короба не меняют маршрут партии: переход всегда идёт по включённым этапам
- На этапе «Короба» их содержимое редактируется, но поставка и сами готовые короба вручную не удаляются
- Статусы `open/closed` сохранены в БД только для совместимости и не используются в UI/логике
...
- Общая галочка находится в шапке таблицы заказов; отдельной второй строки шапки нет
- При выборе заказов снизу по центру появляется компактная плавающая панель со счётчиком и массовыми действиями: печать, перенос и лист подбора
- «Лист подбора» формируется по выбранным заказам: открытие PDF или скачивание XLSX со встроенными фотографиями и данными WB-стикеров
- Отгрузка доступна только целиком для родительской поставки; на уровне одного или выбранных заказов её нет
- Крестик справа закрывает массовую панель и снимает все выбранные галочки
- У каждого заказа кнопка переноса расположена слева от существующей кнопки печати
- При переносе можно выбрать другую активную поставку или создать новую и сразу перенести в неё заказ(ы)
- Имя новой FBS-поставки по умолчанию: `Название магазина ДД.ММ.ГГГГ`
- Баркод конкретного варианта берётся напрямую из FBS-заказа (`skus`); по нему в `products.sizes[].skus` определяется размер, а фото/название/бренд/артикул берутся из карточки `products`
- Для старых строк поддерживается fallback из полного ответа `fbs_orders.data.skus`, если отдельная колонка `skus` пуста; новые синки сохраняют `skus` напрямую, размер проверяется также по `chrtId`
- Если размер не найден, FBS показывает явное предупреждение и предлагает синхронизировать товары выбранного магазина; ручное вычисление WB basket не используется
- В меню «Новые» каждая команда открывает соответствующую вкладку модалки: создание новой или добавление к созданной поставке
...
- Один баркод занимает одну строку внутри короба; повторный скан атомарно увеличивает количество
- Количество меняется только через карандаш → локальный ввод → галочка сохранения
- Корзина удаляет всю позицию только после подтверждения
- Режим «Авто» запоминает последний выбор в `localStorage`
- Карточки поставок имеют единую высоту и нижнюю панель действий; скачивание не выходит за рамку карточки
- Статус карточки явно показывает номер поставки либо «Не передана в логистику»
- Для поставки, созданной после завершения партии, доступна ручная передача в формирующийся рейс
- Передача идемпотентна; после неё количества коробов и единиц автоматически синхронизируются с логистикой
- Перед скачиванием Excel всей поставки или отдельного короба открывается единый выбор колонок
- Скачать короба можно также прямо из списка партий кнопкой слева от редактирования: доступен мультивыбор поставок и вариант `Все поставки`
- Одна выбранная поставка скачивается как XLSX; несколько — ZIP с отдельным XLSX для каждой (`ALL` для всех, `CUSTOM` для выборочных)
- Пустая выбранная поставка подсвечивается красным и блокирует переход к выбору колонок; переход также недоступен без выбора
- Первые 5 колонок обязательны; артикулы и характеристики можно включать/отключать, последний выбор хранится в `localStorage`
- В Excel всей поставки коробы всегда идут по возрастанию `box_number`
- SQL: `supabase/patch_box_item_barcode_unique.sql`
- SQL: `supabase/patch_late_supply_logistics.sql`
...
- **Скан / + Добавить** → оптимистичный UI + атомарный `incrementBoxItem`; повторный баркод увеличивает `qty`
...
- Нельзя удалить поставку или готовые короба внутри; содержимое коробов редактировать можно
- Идентификация: `fulfillment_supplies.source_item_id` с fallback по партии/складу для старых данных
- Удалять поставки «Готовые короба» можно только через этап Приёмки (крестик в строке)
- SQL: `supabase/patch_ready_boxes_sync.sql`
...
- Админская страница `/tz-prompts` разделена на табы «Задачи» и «Промпты ТЗ»; выбранный таб запоминается.
- Короткие задачи добавляются по одной, выводятся сверху вниз и сохраняются в `tz_tasks`.
- Выполненную задачу можно отметить галочкой: текст становится серым и зачёркнутым.
- Таблица и RLS для задач находятся в `supabase/patch_tz_tasks.sql`; доступ разрешён только `superadmin`.
