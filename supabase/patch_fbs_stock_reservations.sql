...
begin
  ...
  select item.* into v_item
  from public.fbs_orders order_row
  join public.fulfillment_box_items item
    on item.account_id = order_row.account_id
   and item.box_id = new.active_box_id
   and item.barcode in (
     select value from jsonb_array_elements_text(coalesce(order_row.skus, '[]'::jsonb)) value
   )
...
declare
  v_box_id uuid;
begin
  select active_box_id into v_box_id
  from public.fbs_marking_sessions
