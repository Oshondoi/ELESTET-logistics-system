...
begin
  ...
  if v_is_accepted and not v_was_accepted then
    ...
    update public.fbs_dispatch_attempts attempt
    set accepted_at = coalesce(attempt.accepted_at, v_seen_at),
        acceptance_source = coalesce(attempt.acceptance_source, 'wb_status_seen'),
        acceptance_is_estimated = case when attempt.accepted_at is null then true else attempt.acceptance_is_estimated end,
        updated_at = timezone('utc', now())
    where attempt.store_id = new.store_id
      and attempt.wb_order_id = new.wb_order_id
      and attempt.wb_supply_id = new.supply_id;
  else
    update public.fbs_dispatch_events event
