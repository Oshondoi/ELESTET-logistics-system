...
-- 1. Убираем уникальный constraint (store_id, shipment_number)
--    и добавляем уникальный (account_id, shipment_number)
...
-- 2. Пересчитываем существующие shipment_number глобально по account_id
--    (каждой строке в порядке created_at присваиваем новый уникальный номер)
do $$
declare
  r record;
  counter integer;
...
-- 3. Обновляем функцию add_trip_line: нумерация по account_id
...
begin
  ...
  insert into public.trip_lines (
    ...
    arrival_date, reception_date, status, payment_status, comment
  )
  values (
    ...
    p_arrival_date, p_reception_date, p_status, p_payment_status,
    coalesce(p_comment, '')
  )
