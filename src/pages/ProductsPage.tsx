...
export const ProductsPage = ({ stores, activeAccountId, selectedStoreId, onStoreChange }: ProductsPageProps) => {
  ...
  const [defectStoreId, setDefectStoreId] = useState(() => selectedStoreId || stores[0]?.id || '')
  const [defectStoreDropdownOpen, setDefectStoreDropdownOpen] = useState(false)
  const defectStoreDropdownRef = useRef<HTMLDivElement | null>(null)

  // Если selectedStoreId изменился и defectStoreId ещё не задан — подхватить
  useEffect(() => {
    if (!defectStoreId && selectedStoreId) setDefectStoreId(selectedStoreId)
  }, [selectedStoreId, defectStoreId])

  // Если stores загрузились — выбрать первый если ничего не выбрано
  useEffect(() => {
  ...
  const loadDefects = useCallback(async (storeId: string, accountId: string) => {
    ...
    try {
      const data = await fetchMarkingDefectsByStore(storeId, accountId)
      setDefects(data)
    } catch {
      // silent
    } finally {
  ...
  useEffect(() => {
    if (activeTab !== 'defects' || !defectStoreId) return
    void loadDefects(defectStoreId, activeAccountId)
  }, [activeTab, defectStoreId, activeAccountId, loadDefects])
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab === 'defects' && (
        <>
          {/* Toolbar браков — все магазины */}
          <Card className="rounded-3xl p-2.5">
            <div ref={defectStoreDropdownRef} className="relative inline-block">
              <button
                type="button"
                ...
                <div className="absolute left-0 top-full z-20 mt-1.5 min-w-[180px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
                  {stores.map((s) => (
                    <button key={s.id} type="button"
                      onClick={() => { setDefectStoreId(s.id); setDefectStoreDropdownOpen(false) }}
                      className={`flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-sm transition hover:bg-slate-50 ${s.id === defectStoreId ? 'font-semibold text-blue-600' : 'text-slate-700'}`}
                    >
                      {s.id === defectStoreId
              ...
              {isLoadingDefects ? (
                ...
                <>
                  <div className="flex flex-col">
                    <span className="text-[10px] font-medium uppercase tracking-wide text-slate-400">Позиций</span>
                    <span className="text-sm font-semibold text-slate-900">{defects.length}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] font-medium uppercase tracking-wide text-slate-400">Всего браков</span>
                    <span className="text-sm font-semibold text-rose-500">{defects.reduce((s, d) => s + d.qty_defect, 0)}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] font-medium uppercase tracking-wide text-slate-400">Баркодов</span>
                    <span className="text-sm font-semibold text-slate-900">{new Set(defects.map(d => d.barcode).filter(Boolean)).size}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] font-medium uppercase tracking-wide text-slate-400">Партий</span>
                    <span className="text-sm font-semibold text-slate-900">{new Set(defects.map(d => d.batch_id)).size}</span>
                  </div>
                </>
              )}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    ...
                    <th className="px-4 py-2.5 text-right text-xs font-medium text-slate-500">Кол-во брак</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {!isLoadingDefects && defects.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-4 py-16 text-center text-sm text-slate-400">Браки не зафиксированы</td>
                    </tr>
                  )}
                  {defects.map((d) => (
                    <tr key={d.id} className="align-middle transition-colors hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-slate-700">{d.barcode ?? <span className="text-slate-300">—</span>}</td>
                      <td className="px-4 py-3 text-right text-sm font-semibold text-rose-500">{d.qty_defect}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Card className="rounded-3xl">
          <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">Добавьте API ключ Wildberries в настройках магазина, чтобы синхронизировать товары</p>
            </div>
          </div>
        </Card>
      )}
