...
БД: fulfillment_otk_log_history, fulfillment_marking_log_history
  ...
  RLS: SELECT USING true; INSERT WITH CHECK uid()=user_id`,
    ...
    },
    {
      ...
      category: 'WB API',
      description: 'WB присваивает коробам собственные коды. Порядок packageCode в публичном API не используется для привязки к номерам коробов ELESTET: API не предоставляет подтверждённого стабильного номера позиции. Эталон привязки — однолистный Excel WB с колонками «ШК короба» и «ШК короба для печати в стороннем сервисе».',
    },
    {
...
ШАБЛОН КОРОБОВ (Скачать короба):
  ...
  Источник: наши коробы из фулфилмента + проверенная привязка из загруженного Excel WB
  Соответствие: первая заполненная пара Excel → короб №1, вторая → короб №2 и далее; пустые строки игнорируются
  Требует: загрузить ШК коробов WB через Excel в поставке Фулфилмента
...
СЦЕНАРИЙ:
  ...
  3. Скачали Excel WB с ШК коробов и загрузили его в ELESTET
  4. Во вкладке «Экспорт» выбрали «ШК короба WB»
  5. Загрузили готовый файл в ЛК WB
...
Файлы: src/lib/wbExcelExport.ts, src/components/ui/TripLineStickerCell.tsx`,
    },
    {
      ...
      category: 'WB API',
      description: 'Числовой ID поставки FBW в системе WB (напр. 39201279). Вводится вручную после создания поставки в ЛК WB. Нужен для безопасной синхронизации метаданных поставки и связывает Excel-привязку ШК с нужной поставкой.',
    },
    {
