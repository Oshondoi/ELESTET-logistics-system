...
const BOX_EXPORT_PICKER_COLUMNS = SYSTEM_BOX_EXPORT_COLUMNS
...
interface FulfillmentPageProps {
  ...
  onAddTripLine?: (tripId: string, values: TripLineFormValues, fulfillmentSupplyId?: string | null) => Promise<TripLine>
...
const buildBoxExportRows = async ({
  ...
  if (accountShortId == null || batchShortId == null || supplyNumber == null) {
    throw new Error('Не удалось сформировать ШК короба: отсутствует ID компании, партии или поставки')
  }
  ...
  const selectedColumns = SYSTEM_BOX_EXPORT_COLUMNS.filter((column) =>
    column.required || selectedOptional.has(column.key as OptionalBoxExportColumnKey),
  )
  ...
  sortedBoxes.forEach((box) => {
    const boxBarcode = box.barcode || buildFulfillmentBoxBarcode({
      accountShortId,
      batchShortId,
      supplyNumber,
...
const BatchDetailModal = ({
  ...
  const isReadyBoxSupply = (supply: FulfillmentSupplyWithBoxes) => {
    ...
    const warehouseHasLinkedReadySupply = supplies.some((candidate) =>
      !!candidate.source_item_id && candidate.warehouse_name === supply.warehouse_name
    )
    return !warehouseHasLinkedReadySupply && items.some((item) =>
      item.product_name === 'Готовые короба' && item.notes === supply.warehouse_name
    )
  }
  ...
  const handleUpdateReadyBoxesDraft = (id: string, boxes: number) => {
    ...
    if (pendingItemIds.has(id)) {
      const next = items.map((it) => it.id === id ? { ...it, boxes: nextBoxes } : it)
      setItems(next)
      onItemsChanged(next)
    }
    setIsDirty(true)
  ...
  const confirmReadyBoxCountChanges = (changes: { item: FulfillmentItem; boxes: number }[]) => {
    const reductions: string[] = []
    for (const { item, boxes: targetCount } of changes) {
      const supply = supplies.find((s) =>
        s.source_item_id === item.id || (!s.source_item_id && s.warehouse_name === item.notes)
      )
      if (!supply || targetCount >= (item.boxes ?? 0)) continue
      const boxesToDelete = supply.boxes
        .filter((box) => box.box_number > targetCount)
        .sort((a, b) => a.box_number - b.box_number)
      const blocked = boxesToDelete.filter((box) => box.items.length > 0)
      if (blocked.length > 0) {
        setError(`Нельзя уменьшить количество до ${targetCount}. Содержат товары: ${blocked.map((box) => `BOX${box.box_number}`).join(', ')}.`)
        return false
      }
      if (boxesToDelete.length > 0) {
        reductions.push(`${supply.warehouse_name}: ${boxesToDelete.map((box) => `BOX${box.box_number}`).join(', ')}`)
      }
    }
    return reductions.length === 0 || window.confirm(`Будут удалены пустые готовые короба:\n${reductions.join('\n')}\n\nПродолжить?`)
  }
  ...
  const handleSaveReceptionDraft = async () => {
    ...
    try {
      ...
      const finalItems = currentItems.map((it) => {
        ...
        return boxChange ? { ...u, boxes: boxChange.boxes } : u
      })
        ...
        for (const id of currentPendingDelete) {
          ...
          if (itemToDelete?.product_name === 'Готовые короба' && itemToDelete.notes) {
            const matchingSupply = supplies.find((s) =>
              (s.source_item_id === itemToDelete.id || (!s.source_item_id && s.warehouse_name === itemToDelete.notes)) &&
  ...
  const persistLocalSupplies = async () => {
    ...
    // Также сохранить новые короба и позиции у существующих поставок.
        ...
        if (batch.status === 'done' && _autoTripId && batch.store_id) {
          ...
          for (const supply of freshSupplies.filter((s) => !s.trip_line_id)) {
            ...
            const tripLine = onAddTripLine
              ? await onAddTripLine(_autoTripId, tripLineValues, supply.id)
              : await addTripLine(batch.account_id, _autoTripId, tripLineValues, supply.id)
    ...
    try {
      ...
      if (workflowStage === 'logistics' && hasAnyTripSlot && batch.store_id) {
        ...
        for (const supply of freshSupplies.filter((s) => !s.trip_line_id)) {
          ...
          const tripLine = onAddTripLine
            ? await onAddTripLine(tripIdForSupply, tripLineValues, supply.id)
            : await addTripLine(batch.account_id, tripIdForSupply, tripLineValues, supply.id)
              ...
              <>
                            ...
                            <div className="flex min-h-9 flex-shrink-0 items-end justify-between gap-2 px-2 pb-2">
                              <div className="flex items-center gap-1">
                                <button
                                  ...
                                  onClick={() => printSupplyBoxQrs(supply)}
                                  ...
                                  title={totalBoxes === 0 ? 'В поставке нет коробов' : 'Распечатать QR всех коробов поставки'}
                                >
                                  <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9V2h12v7M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/><path d="M18 12h.01"/></svg>
                                </button>
                                <button
                                  type="button"
                  ...
                  {boxExportDialog && (() => {
                    ...
                    return createPortal(
                      ...
                      >
                        <div className="w-full max-w-md rounded-3xl bg-white p-5 shadow-2xl space-y-4" onClick={(event) => event.stopPropagation()}>
                          <div className="flex items-start justify-between gap-4">
                            <div>
                            ...
                            <button
                              ...
                              className="flex h-8 w-8 flex-shrink-0 cursor-pointer items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40"
                              aria-label="Закрыть"
                            >
                              <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
                            </button>
                              ...
                              {(boxExportMode === 'barcodes' ? BARCODE_EXPORT_COLUMNS : BOX_EXPORT_PICKER_COLUMNS).map((column) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`flex h-9 w-full cursor-pointer items-center gap-2 rounded-xl border px-3 text-xs font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
                                      selected
                          ...
                          <div className="flex justify-end gap-2">
                            <button
                              ...
                              disabled={isExportingBoxesExcel}
                              onClick={() => setBoxExportDialog(null)}
                              className="cursor-pointer rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                            >
                              Отмена
                            </button>
                            <button
                              ...
                              className="cursor-pointer rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700 disabled:cursor-wait disabled:opacity-50"
                            >
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                            ...
                            <div className="flex items-center gap-3">
                              {(() => {
                                const isActiveStage = batch.current_stage === 'packing'
                                const canDelete = canManageStageData && (isActiveStage || canSupplyDeleteLocked) && !readyBoxSupply
                                return canDelete ? (
                                  <button
                                ...
                                <div className="flex flex-col h-full">
                                  ...
                                  <div className="flex items-center gap-3 px-5 border-b border-slate-100 bg-slate-50 min-h-[44px]">
                                    ...
                                    <button
                                      type="button"
                                      title="QR-код короба"
                                      onClick={() => setBoxQrDialog({ supplyId: supply.id, boxId: box.id })}
                                      className="flex items-center rounded-full p-1 text-slate-400 transition-colors hover:bg-slate-200 hover:text-slate-600"
                                    >
                                      <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2">
                                        <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><path d="M14 14h3v3h-3zM20 14v3M14 20h3M20 20h1"/>
                                      </svg>
                                    </button>
                                    {/* Экспорт содержимого активного короба в Excel */}
                                    {box.items.length > 0 && (
                                      <button
                                        type="button"
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  setSupplies((prev) => prev.map((s) => s.id === supplyId ? {
                                    ...s,
                                    next_box_number: Math.max(s.next_box_number, nextAvailableBoxNumber),
                        ...
                        if (batch.current_stage !== 'logistics' && batch.store_id) {
                          ...
                          for (const supply of freshSupplies) {
                            ...
                            if (supply.trip_line_id) {
                              ...
                              const tripLine = onAddTripLine
                                ? await onAddTripLine(tripId, tripLineValues, supply.id)
                                : await addTripLine(batch.account_id, tripId, tripLineValues, supply.id)
          ...
          <FulfillmentBoxExcelDialog
            ...
            exportContent={(
              <FulfillmentSupplyExcelExport
                supply={supply}
                onDownloadSystem={(optionalColumns, mode) => downloadSupplyBoxesExcel(supply, optionalColumns, mode)}
                onDownloadWb={async () => {
                  const filename = boxExportFilename(supply, 'boxes').replace(/_Box\.xlsx$/i, '_WB_Box.xlsx')
                  downloadBoxesTemplate(supply, filename)
                }}
              />
            )}
...
export const FulfillmentPage = ({ accountId, accountShortId, accountName = '', stores, trips, warehouses, onEditTripLine, onAddTripLine, onTripCreated, onRefreshTrips, onStoreCreated, canManage = true, canOtkAssign = false, canStageJump = false, canPackingAutoAdd = false, canSupplyDeleteLocked = false, userId = '', userEmail = '', userName = '', initialBatchShortId, onBatchUrlConsumed }: FulfillmentPageProps) => {
  ...
  const handleOpenBatchBoxesExport = async (batch: FulfillmentBatch, action: 'excel' | 'qr' | 'contents' = 'excel') => {
    ...
    try {
      ...
      setBatchExportSource({ batch: fullBatch, supplies, action })
    } catch (error) {
      setBatchExportError(error instanceof Error ? error.message : 'Не удалось загрузить короба партии')
      setBatchExportSource({ batch: { ...batch, items: [] }, supplies: [], action })
    } finally {
      setIsPreparingBatchExportId(null)
      setIsPreparingBatchExportAction(null)
    }
  }
  ...
  const continueBatchExport = () => {
    ...
    const emptySelected = selected.filter((supply) => isBatchExportSupplyEmpty(supply, batchExportSource.action))
    ...
    const allSelected = sortedSelected.length === batchExportSource.supplies.length
    openBatchExportColumns(
      batchExportSource.batch,
      sortedSelected,
      sortedSelected.length === 1 ? null : allSelected ? 'ALL' : 'CUSTOM',
    )
  }

  const handleDownloadBatchBoxes = async (
    selectedColumns: OptionalBoxExportColumnKey[] = batchExportSelectedColumns,
    selectedMode: FulfillmentExcelMode = batchExportMode,
  ) => {
    ...
    try {
      ...
      if (supplies.length === 1) {
        ...
        const data = await buildBoxExportRows({
          ...
          boxes: supply.boxes,
          optionalColumnKeys: selectedColumns,
        })
  ...
  const selectedEmptyBatchExportSupplies = selectedBatchExportSupplies.filter((supply) =>
    isBatchExportSupplyEmpty(supply, batchExportSource?.action ?? 'excel'))
          ...
          <div className="w-full max-w-xl space-y-4 rounded-3xl bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {batchExportSource.supplies.map((supply) => {
                    ...
                    const empty = isBatchExportSupplyEmpty(supply, batchExportSource.action)
                    return (
                      <button
                        key={supply.id}
                        type="button"
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className={`mt-0.5 block text-xs ${selected && empty ? 'font-medium text-red-600' : 'text-slate-400'}`}>
                            {selected && empty
                              ? batchExportSource.action === 'qr' ? 'Короба отсутствуют' : 'Данные коробов отсутствуют'
                              : `${boxes} кор. · ${units} ед.`}
                          </span>
                        </span>
                      </button>
            ...
            {batchExportSource.supplies.length > 0 && (
              <div className="flex items-center justify-between gap-4 border-t border-slate-100 pt-4">
                ...
                >
                  {batchExportSource.action === 'qr' ? 'Открыть PDF' : 'Далее'}
                </button>
              </div>
            )}
          </div>
      ...
      {batchExportTarget && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 p-4" onClick={() => { if (!isExportingBatchBoxes) setBatchExportTarget(null) }}>
          <div className="w-full max-w-xl space-y-4 rounded-3xl bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            {batchExportTarget.supplies.length === 1 ? (
              <FulfillmentSupplyExcelExport
                supply={batchExportTarget.supplies[0]}
                onDownloadSystem={(columns, mode) => handleDownloadBatchBoxes(columns, mode)}
                onDownloadWb={async () => {
                  const supply = batchExportTarget.supplies[0]
                  const store = stores.find((candidate) => candidate.id === batchExportTarget.batch.store_id)
                  const filename = getBoxExportFilename({
                    accountShortId,
                    batchShortId: batchExportTarget.batch.short_id,
                    storeName: store?.name,
                    warehouseName: supply.warehouse_name,
                    mode: 'boxes',
                  }).replace(/_Box\.xlsx$/i, '_WB_Box.xlsx')
                  downloadBoxesTemplate(supply, filename)
                  setBatchExportTarget(null)
                }}
              />
                    ...
                    <td className="px-3 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                      {isArchived ? (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          <button
                            type="button"
                            title="Распечатать QR всех коробов партии"
                            disabled={isPreparingBatchExportId != null}
                            onClick={() => void handleOpenBatchBoxesExport(b, 'qr')}
                          ...
                          >
                            {isPreparingBatchExportId === b.id && isPreparingBatchExportAction === 'qr' ? (
                          ...
                          <button
                            ...
                            className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-xl text-slate-300 transition hover:bg-blue-50 hover:text-blue-500 disabled:cursor-wait disabled:opacity-50"
                          >
                            {isPreparingBatchExportId === b.id && isPreparingBatchExportAction === 'excel' ? (
                              <svg className="h-3.5 w-3.5 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>
                            ) : (
