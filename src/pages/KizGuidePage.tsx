...
const AUDIT_CHANGES = [
  ...
  'Устаревшее ограничение 1000 КМ больше не выдано за общий предел: ELESTET допускает до 5000 КМ в текущей операции эмиссии, а лимит файла печати показывается отдельно.',
  'LP RF — фильтр/обозначение российской лёгкой промышленности, а не универсальный режим всех товарных групп.',
  'Эмиссия ELESTET больше не подставляет LP всем товарам: товарная группа определяется по точному GTIN в Teksher.',
...
const STAGES: Stage[] = [
  ...
  {
    num: 6, icon: '🎟️', title: 'Заказать КИЗы — эмиссия', subtitle: 'В ELESTET — автоматический серийный номер',
    description: 'Эмиссия создаёт уникальные коды для опубликованных товаров. Текущая форма ELESTET использует автоматическое формирование серийных номеров.',
    details: [
      'Текущая операция ELESTET принимает от 1 до 5000 КМ для одного GTIN. Если оператор снова изменит серверный предел, ELESTET покажет исходную ошибку Teksher и не будет притворяться, что заказ создан.',
      'PDF v1.7 описывает до 10 GTIN одной товарной группы и одного ТН ВЭД в составном заказе. Текущая форма ELESTET намеренно создаёт одну проверяемую операцию на один GTIN.',
      'Выберите опубликованный товар и количество кодов.',
      'ELESTET проверяет точный GTIN на сервере, получает его товарную группу из Teksher и использует её extension. Группа LP не подставляется товарам других категорий.',
      'В форме ELESTET серийные номера создаёт Teksher автоматически.',
      'Сценарий с собственными серийными номерами доступен в кабинете Teksher через CSV: один уникальный номер в строке, без заголовка, UTF-8. В текущей кнопке ELESTET он не реализован.',
    ...
    technical: ['Связь «ID операции → extension товарной группы» сохраняется на сервере ELESTET и повторно используется при регистрации нанесения. Ограничение операции эмиссии не является ограничением трансграна, FBO или FBS.'],
    warning: 'Не выбирайте группу вручную и не подменяйте её на LP. Если Teksher не подтвердил товар или extension, ELESTET не создаёт платный заказ.',
  ...
  },
  {
    ...
    warning: 'Лимиты эмиссии и файлов печати не являются лимитом трансграна. При внутренней перевозке по РФ не создавайте повторный трансгран без проверки прежней операции и документов.',
