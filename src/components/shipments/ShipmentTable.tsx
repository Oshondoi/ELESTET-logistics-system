...
export const ShipmentTable = ({ shipments }: ShipmentTableProps) => {
  ...
  useEffect(() => {
    setSelectedIds((current) => current.filter((id) => shipments.some((shipment) => shipment.id === id)))
  }, [shipments])

  const allSelected = useMemo(
    () => shipments.length > 0 && selectedIds.length === shipments.length,
    [selectedIds, shipments.length],
  )
  ...
  const handleToggleAll = () => {
    setSelectedIds(allSelected ? [] : shipments.map((shipment) => shipment.id))
  }

  const handleToggleOne = (shipmentId: string) => {
  ...
  return (
    <Card className="overflow-hidden rounded-3xl">
      <div className="overflow-x-auto">
        <table className="min-w-[1360px] divide-y divide-slate-200 text-[13px]">
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {shipments.map((shipment) => {
              ...
              return (
                <tr
                  key={shipment.id}
                  className={`align-top text-slate-700 transition-colors duration-150 hover:bg-slate-100 ${
                ...
                >
                  <td className="px-3 py-3.5">
                    <input
                      type="checkbox"
                      checked={isSelected}
                      onChange={() => handleToggleOne(shipment.id)}
                      aria-label={`Выбрать поставку ${shipment.tracking_code}`}
                      className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500"
                    />
                  </td>
